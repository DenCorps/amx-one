...
use sp_state_machine::{
    ChildStorageCollection, IndexOperation, IterArgs, OffchainChangesCollection, StorageCollection, StorageIterator,
};
...
pub struct ImportSummary<Block: BlockT> {
    ...
    pub tree_route: Option<mp_blockchain::TreeRoute<Block>>,
...
pub trait BlockImportOperation<Block: BlockT> {
    ...
    fn update_db_storage(&mut self, update: TransactionForSB<Self::State, Block>) -> mp_blockchain::Result<()>;
    ...
    fn update_offchain_storage(&mut self, _offchain_update: OffchainChangesCollection) -> mp_blockchain::Result<()> {
        Ok(())
    }
    ...
    fn update_transaction_index(&mut self, index: Vec<IndexOperation>) -> mp_blockchain::Result<()>;
}
    ...
    Block: BlockT,
{
    ...
    pub fn new(state: State, prefix: Option<&StorageKey>, start_at: Option<&StorageKey>) -> Result<Self, State::Error> {
    ...
    State: StateBackend<HashFor<Block>>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next_pair(&self.state)?.ok().map(|(key, value)| (StorageKey(key), StorageData(value)))
    }
}
...
pub trait StorageProvider<Block: BlockT, B: Backend<Block>> {
    ...
    fn storage(&self, hash: Block::Hash, key: &StorageKey) -> mp_blockchain::Result<Option<StorageData>>;

    /// Given a block's `Hash` and a key, return the value under the hash in that block.
    fn storage_hash(&self, hash: Block::Hash, key: &StorageKey) -> mp_blockchain::Result<Option<Block::Hash>>;
    ...
    ) -> mp_blockchain::Result<KeysIter<B::State, Block>>;
    ...
    fn storage_pairs(
        ...
        prefix: Option<&StorageKey>,
        start_key: Option<&StorageKey>,
    ) -> mp_blockchain::Result<PairsIter<B::State, Block>>;
    ...
    fn child_storage(
        ...
        child_info: &ChildInfo,
        key: &StorageKey,
    ) -> mp_blockchain::Result<Option<StorageData>>;
...
pub trait Backend<Block: BlockT>: AuxStore + Send + Sync {
    ...
    fn commit_operation(&self, transaction: Self::BlockImportOperation) -> mp_blockchain::Result<()>;
    ...
    fn pin_block(&self, hash: Block::Hash) -> mp_blockchain::Result<()>;
    ...
    fn remove_leaf_block(&self, hash: Block::Hash) -> mp_blockchain::Result<()>;
