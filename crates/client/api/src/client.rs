...
use crate::{FinalizeSummary, ImportSummary};
...
pub trait BlockchainEvents<Block: BlockT> {
    ...
    ) -> mp_blockchain::Result<StorageEventStream<Block::Hash>>;
}
...
/// This gives the opportunity to perform auxiliary pre-commit actions and optionally
/// enqueue further storage write operations to be atomically performed on commit.
pub type OnFinalityAction<Block> = Box<dyn (Fn(&FinalityNotification<Block>) -> AuxDataOperations) + Send>;
...
pub trait BlockBackend<Block: BlockT> {
    ...
    fn block_body(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<<Block as BlockT>::Extrinsic>>>;
    ...
    fn block_indexed_body(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<Vec<u8>>>>;

    /// Get full block by hash.
    fn block(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<SignedBlock<Block>>>;

    /// Get block status by block hash.
    fn block_status(&self, hash: Block::Hash) -> mp_blockchain::Result<mp_consensus::BlockStatus>;
    ...
    fn block_hash(&self, number: NumberFor<Block>) -> mp_blockchain::Result<Option<Block::Hash>>;

    /// Get single indexed transaction by content hash.
    ///
    /// Note that this will only fetch transactions
    /// that are indexed by the runtime with `storage_index_transaction`.
    fn indexed_transaction(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<u8>>>;

    /// Check if transaction index exists.
    fn has_indexed_transaction(&self, hash: Block::Hash) -> mp_blockchain::Result<bool> {
...
pub trait ProvideUncles<Block: BlockT> {
    ...
    ) -> mp_blockchain::Result<Vec<Block::Header>>;
}
...
impl fmt::Display for UsageInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "caches: ({} state, {} db overlay), i/o: ({} tx, {} write, {} read, {} avg tx, {}/{} key cache \
             reads/total, {} trie nodes writes)",
...
impl<Block: BlockT> UnpinHandleInner<Block> {
    ...
    pub fn new(hash: Block::Hash, unpin_worker_sender: TracingUnboundedSender<Block::Hash>) -> Self {
...
impl<Block: BlockT> UnpinHandle<Block> {
    ...
    pub fn new(hash: Block::Hash, unpin_worker_sender: TracingUnboundedSender<Block::Hash>) -> UnpinHandle<Block> {
...
pub struct BlockImportNotification<Block: BlockT> {
    ...
    pub tree_route: Option<Arc<mp_blockchain::TreeRoute<Block>>>,
...
impl<Block: BlockT> BlockImportNotification<Block> {
    ...
    pub fn new(
        ...
        header: Block::Header,
        is_new_best: bool,
        tree_route: Option<Arc<mp_blockchain::TreeRoute<Block>>>,
        unpin_worker_sender: TracingUnboundedSender<Block::Hash>,
    ) -> Self {
        Self {
            hash,
...
impl<B: BlockT> TryFrom<BlockImportNotification<B>> for ChainEvent<B> {
    ...
    fn try_from(n: BlockImportNotification<B>) -> Result<Self, ()> {
        if n.is_new_best { Ok(Self::NewBestBlock { hash: n.hash, tree_route: n.tree_route }) } else { Err(()) }
    }
}
