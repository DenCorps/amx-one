...
use codec::Decode;
use mc_transaction_pool_api::OffchainSubmitTransaction;
use mp_runtime::generic::BlockId;
use mp_runtime::traits::{Block as BlockT, NumberFor};
use parking_lot::RwLock;
use sp_core::offchain::{self, OffchainDbExt, OffchainWorkerExt, TransactionPoolExt};
...
impl<Block: BlockT> ExtensionsFactory<Block> for () {
    fn extensions_for(&self, _: Block::Hash, _: NumberFor<Block>, _capabilities: offchain::Capabilities) -> Extensions {
        Extensions::new()
    }
}
...
impl<Block: BlockT, Ext: Default + Extension> ExtensionsFactory<Block> for ExtensionBeforeBlock<Block, Ext> {
    fn extensions_for(&self, _: Block::Hash, block_number: NumberFor<Block>, _: offchain::Capabilities) -> Extensions {
        let mut exts = Extensions::new();
...
impl<Block: BlockT> ExecutionExtensions<Block> {
    ...
    pub fn extensions(
        ...
        let mut extensions = self.extensions_factory.read().extensions_for(block_hash, block_number, capabilities);
        ...
        if let ExecutionContext::OffchainCall(Some(ext)) = context {
            extensions.register(OffchainWorkerExt::new(offchain::LimitedExternalities::new(capabilities, ext.0)));
        }
