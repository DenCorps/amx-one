...
use mp_blockchain::{CachedHeaderMetadata, HeaderMetadata};
use mp_runtime::generic::BlockId;
use mp_runtime::traits::{Block as BlockT, HashFor, Header as HeaderT, NumberFor, Zero};
...
use sp_state_machine::{
    Backend as StateBackend, ChildStorageCollection, InMemoryBackend, IndexOperation, StorageCollection,
};
...
impl<Block: BlockT> Blockchain<Block> {
    ...
    pub fn set_head(&self, hash: Block::Hash) -> mp_blockchain::Result<()> {
        let header = self.header(hash)?.ok_or_else(|| mp_blockchain::Error::UnknownBlock(format!("{}", hash)))?;

        self.apply_head(&header)
    }

    fn apply_head(&self, header: &<Block as BlockT>::Header) -> mp_blockchain::Result<()> {
        ...
        let best_tree_route = {
            ...
            if &best_hash == header.parent_hash() {
                ...
                let route = mp_blockchain::tree_route(self, best_hash, *header.parent_hash())?;
                Some(route)
            }
        };
    ...
    fn finalize_header(&self, block: Block::Hash, justification: Option<Justification>) -> mp_blockchain::Result<()> {
        let mut storage = self.storage.write();
        storage.finalized_hash = block;

        if justification.is_some() {
            let block = storage.blocks.get_mut(&block).expect("hash was fetched from a block in the db; qed");
    ...
    fn append_justification(&self, hash: Block::Hash, justification: Justification) -> mp_blockchain::Result<()> {
        let mut storage = self.storage.write();

        let block = storage.blocks.get_mut(&hash).expect("hash was fetched from a block in the db; qed");
        ...
        if let Some(stored_justifications) = block_justifications {
            if !stored_justifications.append(justification) {
                return Err(mp_blockchain::Error::BadJustification("Duplicate consensus engine ID".into()));
            }
        } else {
...
impl<Block: BlockT> HeaderBackend<Block> for Blockchain<Block> {
    fn header(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<<Block as BlockT>::Header>> {
    ...
    fn status(&self, hash: Block::Hash) -> mp_blockchain::Result<BlockStatus> {
    ...
    fn number(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<NumberFor<Block>>> {
    ...
    ) -> mp_blockchain::Result<Option<Block::Hash>> {
...
impl<Block: BlockT> HeaderMetadata<Block> for Blockchain<Block> {
    ...
    fn header_metadata(&self, hash: Block::Hash) -> Result<CachedHeaderMetadata<Block>, Self::Error> {
        self.header(hash)?
            ...
            .ok_or_else(|| mp_blockchain::Error::UnknownBlock(format!("header not found: {}", hash)))
    }
...
impl<Block: BlockT> blockchain::Backend<Block> for Blockchain<Block> {
    fn body(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<<Block as BlockT>::Extrinsic>>> {
        Ok(self.storage.read().blocks.get(&hash).and_then(|b| b.extrinsics().map(|x| x.to_vec())))
    }

    fn justifications(&self, hash: Block::Hash) -> mp_blockchain::Result<Option<Justifications>> {
        Ok(self.storage.read().blocks.get(&hash).and_then(|b| b.justifications().cloned()))
    }

    fn last_finalized(&self) -> mp_blockchain::Result<Block::Hash> {
        Ok(self.storage.read().finalized_hash)
    }

    fn leaves(&self) -> mp_blockchain::Result<Vec<Block::Hash>> {
    ...
    ) -> mp_blockchain::Result<Vec<Block::Hash>> {
        Ok(self.storage.read().leaves.displaced_by_finalize_height(block_number).leaves().cloned().collect::<Vec<_>>())
    }

    fn children(&self, _parent_hash: Block::Hash) -> mp_blockchain::Result<Vec<Block::Hash>> {
        unimplemented!()
    }

    fn indexed_transaction(&self, _hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<u8>>> {
        unimplemented!("Not supported by the in-mem backend.")
    }

    fn block_indexed_body(&self, _hash: Block::Hash) -> mp_blockchain::Result<Option<Vec<Vec<u8>>>> {
        unimplemented!("Not supported by the in-mem backend.")
    }
}
...
impl<Block: BlockT> backend::AuxStore for Blockchain<Block> {
    ...
    fn get_aux(&self, key: &[u8]) -> mp_blockchain::Result<Option<Vec<u8>>> {
    ...
    Block::Hash: Ord,
{
    ...
    ) -> mp_blockchain::Result<Block::Hash> {
        check_genesis_storage(&storage)?;

        let child_delta = storage.children_default.values().map(|child_content| {
            (&child_content.child_info, child_content.data.iter().map(|(k, v)| (k.as_ref(), Some(v.as_ref()))))
        });
    ...
    Block::Hash: Ord,
{
    ...
    fn state(&self) -> mp_blockchain::Result<Option<&Self::State>> {
    ...
    fn set_block_data(
        ...
        self.pending_block = Some(PendingBlock { block: StoredBlock::new(header, body, justifications), state });
        Ok(())
    }
    ...
    fn reset_storage(&mut self, storage: Storage, state_version: StateVersion) -> mp_blockchain::Result<Block::Hash> {
        self.apply_storage(storage, true, state_version)
    }

    fn insert_aux<I>(&mut self, ops: I) -> mp_blockchain::Result<()>
    where
    ...
    fn mark_finalized(&mut self, hash: Block::Hash, justification: Option<Justification>) -> mp_blockchain::Result<()> {
        self.finalized_blocks.push((hash, justification));
        Ok(())
    }

    fn mark_head(&mut self, hash: Block::Hash) -> mp_blockchain::Result<()> {
        assert!(self.pending_block.is_none(), "Only one set block per operation is allowed");
        self.set_head = Some(hash);
        Ok(())
    }

    fn update_transaction_index(&mut self, _index: Vec<IndexOperation>) -> mp_blockchain::Result<()> {
        Ok(())
    }
}
    ...
    Block::Hash: Ord,
{
    ...
    fn begin_operation(&self) -> mp_blockchain::Result<Self::BlockImportOperation> {
    ...
    fn commit_operation(&self, operation: Self::BlockImportOperation) -> mp_blockchain::Result<()> {
    ...
    fn finalize_block(&self, hash: Block::Hash, justification: Option<Justification>) -> mp_blockchain::Result<()> {
    ...
    fn state_at(&self, hash: Block::Hash) -> mp_blockchain::Result<Self::State> {
        if hash == Default::default() {
            return Ok(Self::State::default());
        }

        self.states.read().get(&hash).cloned().ok_or_else(|| mp_blockchain::Error::UnknownBlock(format!("{}", hash)))
    }
    ...
    ) -> mp_blockchain::Result<(NumberFor<Block>, HashSet<Block::Hash>)> {
        Ok((Zero::zero(), HashSet::new()))
    }

    fn remove_leaf_block(&self, _hash: Block::Hash) -> mp_blockchain::Result<()> {
        Ok(())
    }
...
pub fn check_genesis_storage(storage: &Storage) -> mp_blockchain::Result<()> {
    if storage.top.iter().any(|(k, _)| well_known_keys::is_child_storage_key(k)) {
        return Err(mp_blockchain::Error::InvalidState);
    }

    if storage.children_default.keys().any(|child_key| !well_known_keys::is_child_storage_key(child_key)) {
        return Err(mp_blockchain::Error::InvalidState);
    }
...
mod tests {
    use mp_api::HeaderT;
    use mp_blockchain::Backend;
    use mp_runtime::{ConsensusEngineId, Justifications};
    use substrate_test_runtime::{Block, Header, H256};

    use crate::in_mem::Blockchain;
    use crate::NewBlockState;
    ...
    fn store_duplicate_justifications_is_forbidden() {
        ...
        assert!(matches!(
            ...
            Err(mp_blockchain::Error::BadJustification(_)),
        ));
    }
}
