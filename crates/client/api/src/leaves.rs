...
use mp_blockchain::{Error, Result};
use mp_runtime::traits::AtLeast32Bit;
    ...
    N: std::fmt::Debug + Clone + AtLeast32Bit + Decode + Encode,
{
    ...
    pub fn displaced_by_finalize_height(&self, number: N) -> FinalizationOutcome<H, N> {
        ...
        FinalizationOutcome { removed: below_boundary.map(|(k, v)| (k.clone(), v.clone())).collect() }
    }
    ...
    pub fn revert(&mut self, best_hash: H, best_number: N) {
        ...
        for (hash, number) in &items {
            if number.0 > best_number {
                assert!(self.remove_leaf(number, hash), "item comes from an iterator over storage; qed",);
            }
        }

        let best_number = Reverse(best_number);
        let leaves_contains_best = self.storage.get(&best_number).map_or(false, |hashes| hashes.contains(&best_hash));
...
mod tests {
    ...
    fn finalization_works() {
        ...
        assert_eq!(outcome.removed, [(Reverse(10), vec![10_2])].into_iter().collect::<BTreeMap<_, _>>(),);
