...
use sc_utils::id_sequence::SeqID as SubscriberId;
use sc_utils::pubsub::{Dispatch, Subscribe, Unsubscribe};
...
pub(super) struct Registry {
    ...
    pub(super) child_listeners:
        HashMap<StorageKey, (HashMap<StorageKey, FnvHashSet<SubscriberId>>, FnvHashSet<SubscriberId>)>,
...
impl<'a> Subscribe<SubscribeOp<'a>> for Registry {
    fn subscribe(&mut self, subs_op: SubscribeOp<'a>, subs_id: SubscriberId) {
        ...
        let keys = Self::listen_from(subs_id, filter_keys.as_ref(), &mut self.listeners, &mut self.wildcard_listeners);

        let child_keys = filter_child_keys.map(|filter_child_keys| {
            filter_child_keys
                .iter()
                .map(|(c_key, o_keys)| {
                    let (c_listeners, c_wildcards) = self.child_listeners.entry(c_key.clone()).or_default();

                    (c_key.clone(), Self::listen_from(subs_id, o_keys.as_ref(), &mut *c_listeners, &mut *c_wildcards))
                })
                .collect()
        });
        ...
        if self.sinks.insert(subs_id, SubscriberSink::new(subs_id, keys, child_keys)).is_some() {
            log::warn!(
                "The `subscribe`-method has been passed a non-unique subs_id (in `mc-client-api::notifications`)"
            );
        }
    }
}
...
impl Registry {
    fn remove_subscriber(&mut self, subscriber: SubscriberId) -> Option<(Keys, ChildKeys)> {
        ...
        Self::remove_subscriber_from(subscriber, &sink.keys, &mut self.listeners, &mut self.wildcard_listeners);
...
impl<'a> std::fmt::Debug for PrintChildKeys<'a> {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
        if let Some(map) = self.0 {
            fmt.debug_map().entries(map.iter().map(|(key, values)| (HexDisplay::from(key), PrintKeys(values)))).finish()
        } else {
            write!(fmt, "None")
        }
    }
