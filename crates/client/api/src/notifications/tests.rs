...
use mp_runtime::testing::{Block as RawBlock, ExtrinsicWrapper, H256 as Hash};

use super::*;

type TestChangeSet =
    (Vec<(StorageKey, Option<StorageData>)>, Vec<(StorageKey, Vec<(StorageKey, Option<StorageData>)>)>);
...
impl From<TestChangeSet> for StorageChangeSet {
    fn from(changes: TestChangeSet) -> Self {
        ...
        let child_filters = Some([(StorageKey(vec![4]), None), (StorageKey(vec![5]), None)].iter().cloned().collect());
        StorageChangeSet {
...
fn triggering_change_should_notify_wildcard_listeners() {
    ...
    assert_eq!(
        ...
        (
            Hash::from_low_u64_be(1),
            (
                vec![(StorageKey(vec![2]), Some(StorageData(vec![3]))), (StorageKey(vec![3]), None),],
                vec![(
                    StorageKey(vec![4]),
                    vec![(StorageKey(vec![5]), Some(StorageData(vec![4]))), (StorageKey(vec![6]), None),]
                )]
            )
                .into()
        )
...
fn should_only_notify_interested_listeners() {
    ...
    assert_eq!(
        ...
        (Hash::from_low_u64_be(1), (vec![(StorageKey(vec![2]), Some(StorageData(vec![3]))),], vec![]).into())
    );
    assert_eq!(
        recv3.next().map(StorageNotification::into_fields).unwrap(),
        (
            Hash::from_low_u64_be(1),
            (vec![], vec![(StorageKey(vec![4]), vec![(StorageKey(vec![5]), Some(StorageData(vec![4])))]),]).into()
        )
    );
}
