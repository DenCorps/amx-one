...
//! [`BlockBuilder`](mp_block_builder::BlockBuilder).
//!
...
use mp_api::{ApiExt, ApiRef, Core, ProvideRuntimeApi, StorageChanges, StorageProof, TransactionOutcome};
pub use mp_block_builder::BlockBuilder as BlockBuilderApi;
use mp_blockchain::{ApplyExtrinsicFailed, Error};
use mp_runtime::traits::{Block as BlockT, Hash, HashFor, Header as HeaderT, NumberFor, One};
use mp_runtime::{legacy, Digest};
    ...
    RA: ProvideRuntimeApi<Block>,
{
    ...
    fn new_block_at<R: Into<RecordProof>>(
        ...
        record_proof: R,
    ) -> mp_blockchain::Result<BlockBuilder<Block, RA, B>>;

    /// Create a new block, built on the head of the chain.
    fn new_block(&self, inherent_digests: Digest) -> mp_blockchain::Result<BlockBuilder<Block, RA, B>>;
}
    ...
    B: backend::Backend<Block>,
{
    ...
    pub fn new(
        ...
        Ok(Self { parent_hash, extrinsics: Vec::new(), api, version, backend, estimated_header_size })
    }
    ...
    pub fn push(&mut self, xt: <Block as BlockT>::Extrinsic) -> Result<(), Error> {
        ...
        self.api.execute_in_transaction(|api| {
            let res = if version < 6 {
                ...
                api.apply_extrinsic_with_context(parent_hash, ExecutionContext::BlockConstruction, xt.clone())
            };
    ...
    pub fn build(mut self) -> Result<BuiltBlock<Block, backend::StateBackendFor<B, Block>>, Error> {
        let header = self.api.finalize_block_with_context(self.parent_hash, ExecutionContext::BlockConstruction)?;

        debug_assert_eq!(
            header.extrinsics_root().clone(),
            HashFor::<Block>::ordered_trie_root(
                self.extrinsics.iter().map(Encode::encode).collect(),
                mp_runtime::StateVersion::V0,
            ),
        );
        ...
        let storage_changes =
            self.api.into_storage_changes(&state, self.parent_hash).map_err(mp_blockchain::Error::StorageChanges)?;

        Ok(BuiltBlock { block: <Block as BlockT>::new(header, self.extrinsics), storage_changes, proof })
    }
...
mod tests {
    ...
    use substrate_test_runtime_client::runtime::ExtrinsicBuilder;
    ...
    fn block_building_storage_proof_does_not_include_runtime_by_default() {
        ...
        let backend = sp_state_machine::create_proof_check_backend::<Blake2Hasher>(genesis_state_root, proof).unwrap();
