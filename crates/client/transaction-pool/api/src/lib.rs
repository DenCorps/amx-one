...
use mp_runtime::traits::{Block as BlockT, Member, NumberFor};
...
/// See `mp_runtime::ValidTransaction` for details about every field.
...
pub enum ChainEvent<B: BlockT> {
    ...
    NewBestBlock {
        ...
        tree_route: Option<Arc<mp_blockchain::TreeRoute<B>>>,
    },
...
impl<TPool: LocalTransactionPool> OffchainSubmitTransaction<TPool::Block> for TPool {
    fn submit_at(&self, at: &BlockId<TPool::Block>, extrinsic: <TPool::Block as BlockT>::Extrinsic) -> Result<(), ()> {
        log::debug!(target: LOG_TARGET, "(offchain call) Submitting a transaction to the pool: {:?}", extrinsic);

        let result = self.submit_local(at, extrinsic);

        result.map(|_| ()).map_err(|e| {
            log::warn!(target: LOG_TARGET, "(offchain call) Error submitting a transaction to the pool: {}", e)
        })
    }
}
