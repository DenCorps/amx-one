...
use syn::token::{Comma, Gt, Lt, PathSep};
use syn::{
    parse_quote, Attribute, Error, Expr, ExprBlock, ExprCall, ExprPath, FnArg, Item, ItemFn, ItemMod, Pat, Path,
    PathArguments, PathSegment, Result, ReturnType, Signature, Stmt, Token, Type, TypePath, Visibility, WhereClause,
};
        ...
        let (verify_stmts, last_stmt) = match item_fn.sig.output {
            ...
            ReturnType::Type(_, _) => {
                ...
                if i + 1 >= item_fn.block.stmts.len() {
                    return Err(Error::new(
                        item_fn.block.span(),
                        "Benchmark `#[block]` or `#[extrinsic_call]` item cannot be the last statement of your \
                         benchmark function definition if you have defined a return type. You should return something \
                         compatible with Result<(), BenchmarkError> (i.e. `Ok(())`) as the last statement or change \
                         your signature to a blank return type.",
                    ));
                }
                let Some(stmt) = item_fn.block.stmts.last() else { return missing_call(item_fn) };
                (Vec::from(&item_fn.block.stmts[(i + 1)..item_fn.block.stmts.len() - 1]), Some(stmt.clone()))
            }
        };
...
pub fn benchmarks(attrs: TokenStream, tokens: TokenStream, instance: bool) -> syn::Result<TokenStream> {
    // gather module info
    ...
    let mod_attrs: Vec<&Attribute> =
        module.attrs.iter().filter(|attr| !attr.path().is_ident(keywords::BENCHMARKS_TOKEN)).collect();
    ...
    let (_brace, mut content) = module.content.ok_or(syn::Error::new(mod_span, "Module cannot be empty!"))?;
    ...
    let benchmark_fn_metas = content.iter_mut().filter_map(|stmt| {
        ...
        let benchmark_attr = func.attrs.iter().find(|attr| attr.path().is_ident(keywords::BENCHMARK_TOKEN))?;
    ...
    let extra_benchmark_names_str: Vec<String> = extra_benchmark_names.iter().map(|n| n.to_string()).collect();
    let skip_meta_benchmark_names_str: Vec<String> = skip_meta_benchmark_names.iter().map(|n| n.to_string()).collect();
    let mut selected_benchmark_mappings: Vec<TokenStream2> = Vec::new();
    let mut benchmarks_by_name_mappings: Vec<TokenStream2> = Vec::new();
    let test_idents: Vec<Ident> =
        benchmark_names_str.iter().map(|n| Ident::new(format!("test_{}", n).as_str(), Span::call_site())).collect();
...
fn expand_benchmark(
    ...
    let (pre_call, post_call, fn_call_body) = match &benchmark_def.call_def {
        BenchmarkCallDef::ExtrinsicCall { origin, expr_call, attr_span: _ } => {
            ...
            let call_err =
                || syn::Error::new(expr_span, "Extrinsic call must be a function call or `_`").to_compile_error();
        ...
        BenchmarkCallDef::Block { block, attr_span: _ } => (quote!(), quote!(#block), quote!(#block)),
    };

    let vis = benchmark_def.fn_vis;

    // remove #[benchmark] attribute
    let fn_attrs = benchmark_def.fn_attrs.iter().filter(|attr| !attr.path().is_ident(keywords::BENCHMARK_TOKEN));
    ...
    sig.ident = Ident::new(format!("_{}", name.to_token_stream().to_string()).as_str(), Span::call_site());
    let mut fn_param_inputs: Vec<TokenStream2> = param_names.iter().map(|name| quote!(#name: u32)).collect();
