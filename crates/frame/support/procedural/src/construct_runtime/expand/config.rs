...
pub fn expand_outer_config(runtime: &Ident, pallet_decls: &[Pallet], scrate: &TokenStream) -> TokenStream {
    ...
    for decl in pallet_decls {
        if let Some(pallet_entry) = decl.find_part("Config") {
            ...
            let field_name = &Ident::new(&pallet_name.to_string().to_snake_case(), decl.name.span());
            ...
            build_storage_calls.extend(expand_config_build_storage_call(scrate, attr, runtime, decl, field_name));
    ...
    quote! {
        ...
        #[cfg(any(feature = "std", test))]
        impl #scrate::mp_runtime::BuildStorage for GenesisConfig {
            fn assimilate_storage(
                &self,
                storage: &mut #scrate::mp_runtime::Storage,
...
fn expand_config_build_storage_call(
    ...
    quote! {
        #attr
        #scrate::mp_runtime::BuildModuleGenesisStorage::
