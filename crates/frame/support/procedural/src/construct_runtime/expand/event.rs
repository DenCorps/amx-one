...
pub fn expand_outer_event(runtime: &Ident, pallet_decls: &[Pallet], scrate: &TokenStream) -> syn::Result<TokenStream> {
    ...
    for pallet_decl in pallet_decls {
        if let Some(pallet_entry) = pallet_decl.find_part("Event") {
            ...
            if instance.is_some() && generics.params.is_empty() {
                let msg = format!(
                    "Instantiable pallet with no generic `Event` cannot be constructed: pallet `{}` must have generic \
                     `Event`",
                    pallet_name,
                );
...
fn expand_event_conversion(scrate: &TokenStream, pallet: &Pallet, pallet_event: &TokenStream) -> TokenStream {
    let variant_name = &pallet.name;
    let attr = pallet.cfg_pattern.iter().fold(TokenStream::new(), |acc, pattern| {
        let attr = TokenStream::from_str(&format!("#[cfg({})]", pattern.original()))
            .expect("was successfully parsed before; qed");
