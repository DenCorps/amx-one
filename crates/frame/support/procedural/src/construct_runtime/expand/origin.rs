...
pub fn expand_outer_origin(
    ...
    for pallet_decl in pallets.iter().filter(|pallet| pallet.name != SYSTEM_PALLET_NAME) {
        if let Some(pallet_entry) = pallet_decl.find_part("Origin") {
            ...
            if instance.is_some() && generics.params.is_empty() {
                let msg = format!(
                    "Instantiable pallet with no generic `Origin` cannot be constructed: pallet `{}` must have \
                     generic `Origin`",
                    name
                );
            ...
            caller_variants.extend(expand_origin_caller_variant(runtime, pallet_decl, index, instance, generics));
    ...
    let doc_string =
        get_intra_doc_string("Origin is always created with the base filter configured in", &system_path_name);

    let doc_string_none_origin = get_intra_doc_string("Create with system none origin and", &system_path_name);

    let doc_string_root_origin = get_intra_doc_string("Create with system root origin and", &system_path_name);

    let doc_string_signed_origin = get_intra_doc_string("Create with system signed origin and", &system_path_name);
