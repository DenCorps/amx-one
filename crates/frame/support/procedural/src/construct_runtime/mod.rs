...
use frame_support_procedural_tools::{generate_crate_access, generate_crate_access_2018, generate_hidden_includes};
use itertools::Itertools;
use parse::{ExplicitRuntimeDeclaration, ImplicitRuntimeDeclaration, Pallet, RuntimeDeclaration, WhereSection};
    ...
    let res = match definition {
        RuntimeDeclaration::Implicit(implicit_def) => {
            check_pallet_number(input_copy.clone().into(), implicit_def.pallets.len())
                .and_then(|_| construct_runtime_intermediary_expansion(input_copy.into(), implicit_def))
        }
...
fn construct_runtime_final_expansion(definition: ExplicitRuntimeDeclaration) -> Result<TokenStream2> {
    ...
    let system_pallet = pallets.iter().find(|decl| decl.name == SYSTEM_PALLET_NAME).ok_or_else(|| {
        syn::Error::new(
            pallets_token.span.join(),
            "`System` pallet declaration is missing. Please add this line: `System: frame_system::{Pallet, Call, \
             Storage, Config, Event<T>},`",
        )
    })?;
    ...
    let inherent = expand::expand_outer_inherent(&name, &block, &unchecked_extrinsic, &pallets, &scrate);
    ...
    let res = quote!(
        ...
        #[derive(
            Clone, Copy, PartialEq, Eq, #scrate::mp_runtime::RuntimeDebug,
            #scrate::scale_info::TypeInfo
        )]
        pub struct #name;
        impl #scrate::mp_runtime::traits::GetNodeBlockType for #name {
            type NodeBlock = #node_block;
        }
        impl #scrate::mp_runtime::traits::GetRuntimeBlockType for #name {
...
fn decl_all_pallets<'a>(
    ...
    let mut all_features = features_to_names.iter().flat_map(|f| f.0.iter().cloned()).collect::<HashSet<_>>();
    ...
    let all_pallets_reversed_with_system_first = attribute_to_names.iter().map(|(attr, names)| {
        ...
        let names = std::iter::once(&system).chain(names.iter().rev().filter(|n| **n != SYSTEM_PALLET_NAME).cloned());
        quote! {
            #attr
            /// All pallets included in the runtime as a nested tuple of types in reversed order.
...
fn decl_pallet_runtime_setup(runtime: &Ident, pallet_declarations: &[Pallet], scrate: &TokenStream2) -> TokenStream2 {
...
fn decl_static_assertions(runtime: &Ident, pallet_decls: &[Pallet], scrate: &TokenStream2) -> TokenStream2 {
    let error_encoded_size_check = pallet_decls.iter().map(|decl| {
        let path = &decl.path;
        let assert_message = format!(
            "The maximum encoded size of the error type in the `{}` pallet exceeds `MAX_MODULE_ERROR_ENCODED_SIZE`",
            decl.name,
        );
