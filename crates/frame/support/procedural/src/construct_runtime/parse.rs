...
use syn::{token, Attribute, Error, Ident, Path, Result, Token};
...
impl Parse for RuntimeDeclaration {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let pallets = input.parse::<ext::Braces<ext::Punctuated<PalletDeclaration, Token![,]>>>()?;
        ...
        match convert_pallets(pallets.content.inner.into_iter().collect())? {
            PalletsConversion::Implicit(pallets) => {
                Ok(RuntimeDeclaration::Implicit(ImplicitRuntimeDeclaration { name, where_section, pallets }))
            }
            PalletsConversion::Explicit(pallets) => Ok(RuntimeDeclaration::Explicit(ExplicitRuntimeDeclaration {
                name,
                where_section,
                pallets,
                pallets_token,
...
impl Parse for WhereSection {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if let Some(WhereDefinition { ref kind_span, ref kind, .. }) = definitions.first() {
            let msg = format!("`{:?}` was declared above. Please use exactly one declaration for `{:?}`.", kind, kind);
...
impl Parse for PalletDeclaration {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        let pallet_parts = if input.peek(Token![::]) && input.peek3(token::Brace) {
            ...
            && !input.is_empty()
        {
            return Err(input.error("Unexpected tokens, expected one of `::{`, `exclude_parts`, `use_parts`, `=`, `,`"));
        } else {
            None
        };
...
impl Parse for PalletPath {
    fn parse(input: ParseStream) -> Result<Self> {
        let mut res = PalletPath { inner: Path { leading_colon: None, segments: Punctuated::new() } };
...
fn parse_pallet_parts(input: ParseStream) -> Result<Vec<PalletPart>> {
    ...
    for part in pallet_parts.content.inner.iter() {
        if !resolved.insert(part.name()) {
            let msg =
                format!("`{}` was already declared before. Please remove the duplicate declaration", part.name(),);
...
impl Parse for PalletPart {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if !generics.params.is_empty() && !keyword.allows_generic() {
            ...
            let msg = format!(
                "`{}` is not allowed to have generics. Only the following pallets are allowed to have generics: {}.",
                keyword.name(),
                valid_generics,
            );
...
fn remove_kind(input: ParseStream, kind: WhereKind, definitions: &mut Vec<WhereDefinition>) -> Result<WhereDefinition> {
    if let Some(pos) = definitions.iter().position(|d| d.kind == kind) {
        ...
        let msg = format!("Missing associated type for `{:?}`. Add `{:?}` = ... to where section.", kind, kind);
        Err(input.error(msg))
    }
}
...
fn parse_pallet_parts_no_generic(input: ParseStream) -> Result<Vec<PalletPartNoGeneric>> {
    let pallet_parts: ext::Braces<ext::Punctuated<PalletPartNoGeneric, Token![,]>> = input.parse()?;
...
fn convert_pallets(pallets: Vec<PalletDeclaration>) -> syn::Result<PalletsConversion> {
    ...
    let pallets = pallets
        ...
        .map(|pallet| {
            ...
            let available_parts = pallet_parts.iter().map(|part| part.keyword.name()).collect::<HashSet<_>>();
                ...
                SpecifiedParts::Exclude(parts) | SpecifiedParts::Use(parts) => {
                    for part in parts {
                        if !available_parts.contains(part.keyword.name()) {
                            let msg = format!(
                                "Invalid pallet part specified, the pallet `{}` doesn't have the `{}` part. Available \
                                 parts are: {}.",
                                pallet.name,
                                part.keyword.name(),
            ...
            match pallet.specified_parts {
                SpecifiedParts::Exclude(excluded_parts) => pallet_parts.retain(|part| {
                    !excluded_parts.iter().any(|excluded_part| excluded_part.keyword.name() == part.keyword.name())
                }),
                SpecifiedParts::Use(used_parts) => pallet_parts
                    .retain(|part| used_parts.iter().any(|use_part| use_part.keyword.name() == part.keyword.name())),
                SpecifiedParts::All => (),
            }
