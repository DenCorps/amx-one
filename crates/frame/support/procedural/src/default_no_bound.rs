    ...
    let impl_ = match input.data {
        ...
        Data::Enum(enum_) => {
            ...
            match &*default_variants {
                ...
                [default_variant] => {
                    let variant_attrs =
                        default_variant.attrs.iter().filter(|a| a.path().is_ident("default")).collect::<Vec<_>>();
                    ...
                    if let [first_attr, second_attr, additional_attrs @ ..] = &*variant_attrs {
                        let mut err = syn::Error::new(Span::call_site(), "multiple `#[default]` attributes");
                        ...
                        err.extend(
                            [second_attr]
                                .into_iter()
                                .chain(additional_attrs)
                                .map(|variant| syn::Error::new_spanned(variant, "`#[default]` used again here")),
                        );
                ...
                [first, additional @ ..] => {
                    ...
                    err.extend(
                        additional.into_iter().map(|variant| syn::Error::new_spanned(variant, "additional default")),
                    );
        ...
        Data::Union(union_) => {
            return syn::Error::new_spanned(union_.union_token, "Union type not supported by `derive(DefaultNoBound)`")
                .to_compile_error()
                .into();
        }
    };
