...
fn get_cargo_env_var<T: FromStr>(version_env: &str) -> std::result::Result<T, ()> {
    let version =
        std::env::var(version_env).unwrap_or_else(|_| panic!("`{}` is always set by cargo; qed", version_env));
...
pub fn benchmark(_attrs: TokenStream, _tokens: TokenStream) -> TokenStream {
    quote!(compile_error!("`#[benchmark]` must be in a module labeled with #[benchmarks] or #[instance_benchmarks]."))
        .into()
}
...
pub fn block(_attrs: TokenStream, _tokens: TokenStream) -> TokenStream {
    quote!(compile_error!("`#[block]` must be in a benchmark function definition labeled with `#[benchmark]`.")).into()
}
...
/// Derive [`Debug`], if `std` is enabled it uses `frame_support::DebugNoBound`, if `std` is
/// not enabled it just returns `"<wasm:stripped>"`.
...
pub fn crate_to_crate_version(input: TokenStream) -> TokenStream {
    crate_version::crate_to_crate_version(input).unwrap_or_else(|e| e.to_compile_error()).into()
}
...
pub fn storage_alias(_: TokenStream, input: TokenStream) -> TokenStream {
    storage_alias::storage_alias(input.into()).unwrap_or_else(|r| r.into_compile_error()).into()
}
...
/// * `#[cfg(feature = "std")]`
///
/// The macro will implement `mp_runtime::BuildModuleGenesisStorage` using `()` as a second
...
/// `Pallet<T>`, and some optional where clause.
///
/// NOTE: There is also the `mp_runtime::traits::SignedExtension` trait that can be used to add
