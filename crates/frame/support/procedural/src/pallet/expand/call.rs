...
use crate::pallet::parse::call::CallWeightDef;
...
pub fn expand_call(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let new_call_variant_fn_name =
        fn_name.iter().map(|fn_name| quote::format_ident!("new_call_variant_{}", fn_name)).collect::<Vec<_>>();

    let new_call_variant_doc =
        fn_name.iter().map(|fn_name| format!("Create a call with the variant `{}`.", fn_name)).collect::<Vec<_>>();
    ...
    let args_name_stripped = methods
        ...
        .map(|method| {
            method
                .args
                .iter()
                .map(|(_, name, _)| syn::Ident::new(name.to_string().trim_start_matches('_'), name.span()))
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    ...
    let default_docs =
        [syn::parse_quote!(r"Contains one variant per dispatchable that can be called by an extrinsic.")];
    ...
    if let Some(syn::Item::Impl(item_impl)) =
        def.call.as_ref().map(|c| &mut def.item.content.as_mut().expect("Checked by def parser").1[c.index])
    {
