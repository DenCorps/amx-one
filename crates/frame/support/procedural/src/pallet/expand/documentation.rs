...
fn parse_doc_value(attr: &Attribute) -> syn::Result<Option<DocMetaValue>> {
    ...
    match &meta.value {
        ...
        _ => Err(syn::Error::new(attr.span(), "Expected `= \"docs\"` or `= include_str!(\"PATH\")`")),
    }
}
...
pub fn expand_documentation(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let docs =
        match def.item.attrs.iter().filter_map(|v| parse_doc_value(v).transpose()).collect::<syn::Result<Vec<_>>>() {
            Ok(r) => r,
    ...
    let pallet_docs =
        match pallet_docs.into_iter().map(|attr| parse_pallet_doc_value(&attr)).collect::<syn::Result<Vec<_>>>() {
            Ok(docs) => docs,
            Err(err) => return err.into_compile_error(),
        };
