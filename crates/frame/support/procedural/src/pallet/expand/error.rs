...
use crate::pallet::parse::error::VariantField;
...
pub fn expand_error(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let error_token_unique_id = syn::Ident::new(&format!("__tt_error_token_{}", count), def.item.span());
    ...
    let error_item = {
        ...
        if let syn::Item::Enum(item) = item { item } else { unreachable!("Checked by error parser") }
    };
    ...
    quote::quote_spanned!(error.attr_span =>
        ...
        impl<#type_impl_gen> From<#error_ident<#type_use_gen>>
            for #frame_support::mp_runtime::DispatchError
            #config_where_clause
        {
            fn from(err: #error_ident<#type_use_gen>) -> Self {
                ...
                #frame_support::mp_runtime::DispatchError::Module( #frame_support::mp_runtime::ModuleError {
                    index,
