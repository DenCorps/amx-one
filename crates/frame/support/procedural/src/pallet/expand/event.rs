...
pub fn expand_event(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let (event, macro_ident) = if let Some(event) = &def.event {
        ...
        let macro_ident = Ident::new(&format!("__is_event_part_defined_{}", count), def.item.span());
    ...
    // message
    let completed_where_clause = super::merge_where_clauses(&[&event.where_clause, &def.config.where_clause]);
    ...
    let event_item = {
        ...
        if let syn::Item::Enum(item) = item { item } else { unreachable!("Checked by event parser") }
    };
