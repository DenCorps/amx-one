...
pub fn expand_genesis_config(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let (genesis_config, def_macro_ident, std_macro_ident) = if let Some(genesis_config) = &def.genesis_config {
        let def_macro_ident =
            Ident::new(&format!("__is_genesis_config_defined_{}", count), genesis_config.genesis_config.span());

        let std_macro_ident =
            Ident::new(&format!("__is_std_macro_defined_for_genesis_{}", count), genesis_config.genesis_config.span());

        (genesis_config, def_macro_ident, std_macro_ident)
    } else {
        let def_macro_ident = Ident::new(&format!("__is_genesis_config_defined_{}", count), def.item.span());

        let std_macro_ident = Ident::new(&format!("__is_std_enabled_for_genesis_{}", count), def.item.span());
    ...
    let genesis_config_item = &mut def.item.content.as_mut().expect("Checked by def parser").1[genesis_config.index];
    ...
    match genesis_config_item {
        ...
        | syn::Item::Type(syn::ItemType { attrs, .. }) => {
            ...
            attrs.push(syn::parse_quote!( #[cfg_attr(feature = "std", serde(rename_all = "camelCase"))] ));
            attrs.push(syn::parse_quote!( #[cfg_attr(feature = "std", serde(deny_unknown_fields))] ));
            attrs.push(syn::parse_quote!( #[cfg_attr(feature = "std", serde(bound(serialize = "")))] ));
            attrs.push(syn::parse_quote!( #[cfg_attr(feature = "std", serde(bound(deserialize = "")))] ));
            attrs.push(syn::parse_quote!( #[cfg_attr(feature = "std", serde(crate = #serde_crate))] ));
        }
