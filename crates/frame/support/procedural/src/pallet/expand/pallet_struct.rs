...
use crate::pallet::expand::merge_where_clauses;
...
pub fn expand_pallet_struct(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let pallet_item = {
        ...
        if let syn::Item::Struct(item) = item { item } else { unreachable!("Checked by pallet struct parser") }
    };
    ...
    let storage_info_span = def.pallet_struct.without_storage_info.unwrap_or(def.pallet_struct.attr_span);
    ...
    let (storage_version, current_storage_version_ty) = if let Some(v) = def.pallet_struct.storage_version.as_ref() {
    ...
    let whitelisted_storage_idents: Vec<syn::Ident> =
        def.storages.iter().filter_map(|s| s.whitelisted.then_some(s.ident.clone())).collect();
