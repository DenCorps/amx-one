...
use crate::pallet::parse::storage::{Metadata, QueryKind, StorageDef, StorageGenerics};
...
fn counter_prefix_ident(storage_ident: &syn::Ident) -> syn::Ident {
    syn::Ident::new(&format!("_GeneratedCounterPrefixForStorage{}", storage_ident), storage_ident.span())
}
...
fn check_prefix_duplicates(
    ...
    let dup_err =
        syn::Error::new(storage_def.prefix_span(), format!("Duplicate storage prefixes found for `{}`", prefix));
    ...
    if let Metadata::CountedMap { .. } = storage_def.metadata {
        ...
        let counter_dup_err = syn::Error::new(
            ...
            format!(
                "Duplicate storage prefixes found for `{}`, used for counter associated to counted storage map",
                counter_prefix,
            ),
        );
...
pub fn process_generics(def: &mut Def) -> syn::Result<Vec<ResultOnEmptyStructMetadata>> {
    ...
    for storage_def in def.storages.iter_mut() {
        ...
        let default_query_kind: syn::Type = syn::parse_quote!( #frame_support::storage::types::OptionQuery);
        let mut default_on_empty = |value_ty: syn::Type| -> syn::Type {
            if let Some(QueryKind::ResultQuery(error_path, variant_name)) = storage_def.query_kind.as_ref() {
                let on_empty_ident = quote::format_ident!("__Frame_Internal_Get{}Result", storage_def.ident);
        ...
        let set_result_query_type_parameter = |query_type: &mut syn::Type| -> syn::Result<()> {
            if let Some(QueryKind::ResultQuery(error_path, _)) = storage_def.query_kind.as_ref() {
                if let syn::Type::Path(syn::TypePath { path: syn::Path { segments, .. }, .. }) = query_type {
                    ...
                    let msg = format!(
                        "Invalid pallet::storage, unexpected type for query, expected ResultQuery with 1 type \
                         parameter, found `{}`",
        ...
        if let Some(named_generics) = storage_def.named_generics.clone() {
            ...
            match named_generics {
                ...
                StorageGenerics::CountedMap { hasher, key, value, query_kind, on_empty, max_values } => {
                    args.args.push(syn::GenericArgument::Type(hasher));
                    args.args.push(syn::GenericArgument::Type(key));
                    args.args.push(syn::GenericArgument::Type(value.clone()));
                    let mut query_kind = query_kind.unwrap_or_else(|| default_query_kind.clone());
            ...
            if storage_def.use_default_hasher {
                ...
                for hasher_idx in hasher_indices {
                    args.args[hasher_idx] =
                        syn::GenericArgument::Type(syn::parse_quote!( #frame_support::Blake2_128Concat));
                }
            }
            ...
            } else if let Some(QueryKind::ResultQuery(error_path, _)) = storage_def.query_kind.as_ref() {
...
pub fn expand_storages(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let mut errors =
        def.storages.iter().filter_map(|storage_def| check_prefix_duplicates(storage_def, &mut prefix_set).err());
    ...
    let getters = def.storages.iter().map(|storage| {
        if let Some(getter) = &storage.getter {
            let completed_where_clause = super::merge_where_clauses(&[&storage.where_clause, &def.config.where_clause]);
            let docs = storage.docs.iter().map(|d| quote::quote_spanned!(storage.attr_span => #[doc = #d]));
    ...
    let on_empty_structs = on_empty_struct_metadata.into_iter().map(|metadata| {
        ...
        let ResultOnEmptyStructMetadata { name, visibility, value_ty, error_path, variant_name, span } = metadata;
        ...
        let generic_kind = match error_path.segments.last() {
            Some(PathSegment { arguments: PathArguments::AngleBracketed(args), .. }) => {
                let (has_config, has_instance) =
                    args.args.iter().fold((false, false), |(has_config, has_instance), arg| match arg {
                        GenericArgument::Type(Type::Path(TypePath { path: Path { segments, .. }, .. })) => {
                            let maybe_config = segments.first().map_or(false, |seg| seg.ident == "T");
                            let maybe_instance = segments.first().map_or(false, |seg| seg.ident == "I");
