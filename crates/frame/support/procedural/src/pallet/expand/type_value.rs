...
pub fn expand_type_values(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    for type_value in &def.type_values {
        ...
        let fn_ident_renamed =
            syn::Ident::new(&format!("__type_value_for_{}", fn_name_snakecase), type_value.ident.span());

        let type_value_item = {
            let item = &mut def.item.content.as_mut().expect("Checked by def").1[type_value.index];
            if let syn::Item::Fn(item) = item { item } else { unreachable!("Checked by error parser") }
        };
        ...
        let (struct_impl_gen, struct_use_gen) = if type_value.is_generic {
            (def.type_impl_generics(type_value.attr_span), def.type_use_generics(type_value.attr_span))
        } else {
