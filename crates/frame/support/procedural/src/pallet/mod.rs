...
pub use parse::composite::keyword::CompositeKeyword;
pub use parse::Def;
...
pub fn pallet(attr: proc_macro::TokenStream, item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let mut dev_mode = false;
    if !attr.is_empty() {
        if let Ok(_) = syn::parse::<keyword::dev_mode>(attr.clone()) {
            dev_mode = true;
        } else {
            let msg = "Invalid pallet macro call: unexpected attribute. Macro call must be bare, such as \
                       `#[frame_support::pallet]` or `#[pallet]`, or must specify the `dev_mode` attribute, such as \
                       `#[frame_support::pallet(dev_mode)]` or #[pallet(dev_mode)].";
