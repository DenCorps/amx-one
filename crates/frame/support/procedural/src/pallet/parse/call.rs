...
impl CallDef {
    pub fn try_from(
        ...
        if let Some((_, _, for_)) = item_impl.trait_ {
            let msg = "Invalid pallet::call, expected no trait ident as in `impl<..> Pallet<..> { .. }`";
        ...
        for item in &mut item_impl.items {
            if let syn::ImplItem::Fn(method) = item {
                if !matches!(method.vis, syn::Visibility::Public(_)) {
                    let msg = "Invalid pallet::call, dispatchable function must be public: `pub fn`";
                ...
                match method.sig.inputs.first() {
                    ...
                    Some(syn::FnArg::Receiver(_)) => {
                        let msg = "Invalid pallet::call, first argument must be a typed argument, e.g. `origin: \
                                   OriginFor<T>`";
                        return Err(syn::Error::new(method.sig.span(), msg));
                    }
                ...
                if let syn::ReturnType::Type(_, type_) = &method.sig.output {
                    ...
                    let msg = "Invalid pallet::call, require return type DispatchResultWithPostInfo";
                    return Err(syn::Error::new(method.sig.span(), msg));
                }
                ...
                let (mut weight_attrs, mut call_idx_attrs): (Vec<FunctionAttr>, Vec<FunctionAttr>) =
                    helper::take_item_pallet_attrs(&mut method.attrs)?
                        .into_iter()
                        .partition(|attr| if let FunctionAttr::Weight(_) = attr { true } else { false });
                ...
                if weight_attrs.is_empty() && dev_mode {
                    ...
                    let empty_weight: syn::Expr =
                        syn::parse(quote::quote!(0).into()).expect("we are parsing a quoted string; qed");
                ...
                for arg in method.sig.inputs.iter_mut().skip(1) {
                    ...
                    let arg_attrs: Vec<ArgAttrIsCompact> = helper::take_item_pallet_attrs(&mut arg.attrs)?;
