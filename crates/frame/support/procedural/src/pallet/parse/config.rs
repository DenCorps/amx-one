...
impl TryFrom<&syn::TraitItemType> for ConstMetadataDef {
    ...
    fn try_from(trait_ty: &syn::TraitItemType) -> Result<Self, Self::Error> {
        let err = |span, msg| syn::Error::new(span, format!("Invalid usage of `#[pallet::constant]`: {}", msg));
        ...
        let bound = trait_ty
            ...
            .find_map(|b| {
                if let syn::TypeParamBound::Trait(tb) = b {
                    tb.path.segments.last().and_then(|s| if s.ident == "Get" { Some(s) } else { None })
                } else {
                    None
                }
            })
    ...
    if let syn::TraitItem::Type(type_) = trait_item {
        if type_.ident == "RuntimeEvent" {
            ...
            if !type_.generics.params.is_empty() || type_.generics.where_clause.is_some() {
                let msg = "Invalid `type RuntimeEvent`, associated type `RuntimeEvent` is reserved and must haveno \
                           generics nor where_clause";
            ...
            let has_is_type_bound = type_.bounds.iter().any(|s| {
                syn::parse2::<IsTypeBoundEventParse>(s.to_token_stream()).map_or(false, |b| b.0 == *frame_system)
            });

            if !has_is_type_bound {
                let msg = format!(
                    "Invalid `type RuntimeEvent`, associated type `RuntimeEvent` is reserved and must bound: \
                     `IsType<<Self as {}::Config>::RuntimeEvent>`",
                    frame_system,
                );
                return Err(syn::Error::new(type_.span(), msg));
            }

            let from_event_bound =
                type_.bounds.iter().find_map(|s| syn::parse2::<FromEventParse>(s.to_token_stream()).ok());

            let from_event_bound = if let Some(b) = from_event_bound {
                b
            } else {
                let msg = "Invalid `type RuntimeEvent`, associated type `RuntimeEvent` is reserved and must bound: \
                           `From<Event>` or `From<Event<Self>>` or `From<Event<Self, I>>`";
                return Err(syn::Error::new(type_.span(), msg));
            };
            ...
            if from_event_bound.is_generic && (from_event_bound.has_instance != trait_has_instance) {
                let msg = "Invalid `type RuntimeEvent`, associated type `RuntimeEvent` bounds inconsistent \
                           `From<Event..>`. Config and generic Event must be both with instance or without instance";
                return Err(syn::Error::new(type_.span(), msg));
            }
...
impl ConfigDef {
    pub fn try_from(
        ...
        for trait_item in &mut item.items {
            // Parse for event
            has_event_type = has_event_type || check_event_type(frame_system, trait_item, has_instance)?;
            ...
            if type_attrs_const.len() == 1 {
                match trait_item {
                    ...
                    _ => {
                        let msg = "Invalid pallet::constant in pallet::config, expected type trait item";
                        return Err(syn::Error::new(trait_item.span(), msg));
                    }
                }
            }
        ...
        let attr: Option<DisableFrameSystemSupertraitCheck> = helper::take_first_item_pallet_attr(&mut item.attrs)?;

        let disable_system_supertrait_check = attr.is_some();

        let has_frame_system_supertrait = item
            .supertraits
            .iter()
            .any(|s| syn::parse2::<ConfigBoundParse>(s.to_token_stream()).map_or(false, |b| b.0 == *frame_system));
        ...
        if !has_frame_system_supertrait && !disable_system_supertrait_check {
            let found = if item.supertraits.is_empty() {
                ...
                let mut found =
                    item.supertraits.iter().fold(String::new(), |acc, s| format!("{}`{}`, ", acc, quote::quote!(#s)));
                found.pop();
                found.pop();
                found
            };
            ...
            let msg = format!(
                "Invalid pallet::trait, expected explicit `{}::Config` as supertrait, found {}. (try `pub trait \
                 Config: frame_system::Config {{ ...` or `pub trait Config<I: 'static>: frame_system::Config {{ \
