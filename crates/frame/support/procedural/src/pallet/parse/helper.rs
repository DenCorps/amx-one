    ...
    Attr: syn::parse::Parse,
{
    ...
    if let Some(index) =
        attrs.iter().position(|attr| attr.path().segments.first().map_or(false, |segment| segment.ident == "pallet"))
    {
...
///
/// return the instance if found.
pub fn check_type_def_gen_no_bounds(gen: &syn::Generics, span: proc_macro2::Span) -> syn::Result<InstanceUsage> {
...
pub fn check_type_def_optional_gen(gen: &syn::Generics, span: proc_macro2::Span) -> syn::Result<Option<InstanceUsage>> {
    let expected = "expected `` or `T` or `T: Config` or `T, I = ()` or `T: Config<I>, I: 'static = ()`";
...
///
/// return the instance if found.
pub fn check_type_def_gen(gen: &syn::Generics, span: proc_macro2::Span) -> syn::Result<InstanceUsage> {
    let expected = "expected `T` or `T: Config` or `T, I = ()` or `T: Config<I>, I: 'static = ()`";
    pub struct Checker(InstanceUsage);
    impl syn::parse::Parse for Checker {
        fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
            let mut instance_usage = InstanceUsage { span: input.span(), has_instance: false };
...
///
/// return the instance if found.
pub fn check_type_value_gen(gen: &syn::Generics, span: proc_macro2::Span) -> syn::Result<Option<InstanceUsage>> {
