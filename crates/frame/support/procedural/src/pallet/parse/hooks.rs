...
impl HooksDef {
    pub fn try_from(attr_span: proc_macro2::Span, index: usize, item: &mut syn::Item) -> syn::Result<Self> {
        ...
        let item_trait = &item
            ...
            .as_ref()
            .ok_or_else(|| {
                let msg = "Invalid pallet::hooks, expected impl<..> Hooks for Pallet<..>";
        ...
        if item_trait.segments.len() != 1 || item_trait.segments[0].ident != "Hooks" {
            let msg = format!(
                "Invalid pallet::hooks, expected trait to be `Hooks` found `{}`, you can import from \
        ...
        Ok(Self { attr_span, index, instances, has_runtime_upgrade, where_clause: item.generics.where_clause.clone() })
    }
}
