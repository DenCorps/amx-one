...
use composite::keyword::CompositeKeyword;
use composite::CompositeDef;
...
impl Def {
    pub fn try_from(mut item: syn::ItemMod, dev_mode: bool) -> syn::Result<Self> {
        ...
        for (index, item) in items.iter_mut().enumerate() {
            ...
            match pallet_attr {
                ...
                Some(PalletAttr::ExtraConstants(_)) => {
                    extra_constants = Some(extra_constants::ExtraConstantsDef::try_from(index, item)?)
                }
                Some(PalletAttr::Composite(span)) => {
                    let composite = composite::CompositeDef::try_from(span, index, &frame_support, item)?;
                    if composites.iter().any(|def| match (&def.composite_keyword, &composite.composite_keyword) {
                        (CompositeKeyword::FreezeReason(_), CompositeKeyword::FreezeReason(_))
                        | (CompositeKeyword::HoldReason(_), CompositeKeyword::HoldReason(_))
                        | (CompositeKeyword::LockId(_), CompositeKeyword::LockId(_))
                        | (CompositeKeyword::SlashReason(_), CompositeKeyword::SlashReason(_)) => true,
                        _ => false,
                    }) {
                        let msg = format!("Invalid duplicated `{}` definition", composite.composite_keyword);
        ...
        if genesis_config.is_some() != genesis_build.is_some() {
            let msg = format!(
                "`#[pallet::genesis_config]` and `#[pallet::genesis_build]` attributes must be either both used or \
                 both not used, instead genesis_config is {} and genesis_build is {}",
        ...
        let def = Def {
            ...
            pallet_struct: pallet_struct.ok_or_else(|| syn::Error::new(item_span, "Missing `#[pallet::pallet]`"))?,
            hooks,
            call,
            extra_constants,
            genesis_config,
    ...
    fn check_event_usage(&self) -> syn::Result<()> {
        match (self.config.has_event_type, self.event.is_some()) {
            (true, false) => {
                let msg = "Invalid usage of RuntimeEvent, `Config` contains associated type `RuntimeEvent`, but enum \
                           `Event` is not declared (i.e. no use of `#[pallet::event]`). Note that type `RuntimeEvent` \
                           in trait is reserved to work alongside pallet event.";
                Err(syn::Error::new(proc_macro2::Span::call_site(), msg))
            }
            (false, true) => {
                let msg = "Invalid usage of RuntimeEvent, `Config` contains no associated type `RuntimeEvent`, but \
                           enum `Event` is declared (in use of `#[pallet::event]`). An RuntimeEvent associated type \
                           must be declare on trait `Config`.";
                Err(syn::Error::new(proc_macro2::Span::call_site(), msg))
            }
            _ => Ok(()),
        }
    ...
    fn check_instance_usage(&self) -> syn::Result<()> {
        ...
        let mut errors = instances.into_iter().filter_map(|instances| {
            ...
            let msg = if self.config.has_instance {
                ...
                "Invalid generic declaration, trait is defined without instance but generic use some"
            };
    ...
    pub fn trait_use_generics(&self, span: proc_macro2::Span) -> proc_macro2::TokenStream {
        if self.config.has_instance { quote::quote_spanned!(span => <I>) } else { quote::quote_spanned!(span => ) }
    }
    ...
    pub fn type_use_generics(&self, span: proc_macro2::Span) -> proc_macro2::TokenStream {
        if self.config.has_instance { quote::quote_spanned!(span => T, I) } else { quote::quote_spanned!(span => T) }
    }
}
