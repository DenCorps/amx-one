...
impl PalletStructDef {
    pub fn try_from(attr_span: proc_macro2::Span, index: usize, item: &mut syn::Item) -> syn::Result<Self> {
        ...
        for attr in struct_attrs {
            match attr {
                ...
                PalletStructAttr::WithoutStorageInfoTrait(span) if without_storage_info.is_none() => {
                    without_storage_info = Some(span);
                }
                PalletStructAttr::StorageVersion { storage_version, .. } if storage_version_found.is_none() => {
        ...
        let instances = vec![helper::check_type_def_gen_no_bounds(&item.generics, item.ident.span())?];
