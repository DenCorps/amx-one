    ...
    fn from_attrs(attrs: Vec<PalletStorageAttr>) -> syn::Result<Self> {
        ...
        for attr in attrs {
            match attr {
                ...
                PalletStorageAttr::StorageName(name, ..) if rename_as.is_none() => rename_as = Some(name),
                ...
                attr => return Err(syn::Error::new(attr.attr_span(), "Invalid attribute: Duplicate attribute")),
            }
        }
...
impl StorageGenerics {
    ...
    fn metadata(&self) -> syn::Result<Metadata> {
        let res = match self.clone() {
            ...
            Self::NMap { keygen, value, .. } => Metadata::NMap { keys: collect_keys(&keygen)?, keygen, value },
        };
...
fn check_generics(
    ...
    for (gen_name, gen_binding) in map {
        if !mandatory_generics.contains(&gen_name.as_str()) && !optional_generics.contains(&gen_name.as_str()) {
            let msg = format!(
...
fn process_named_generics(
    ...
    let generics = match storage {
        StorageKind::Value => {
            check_generics(&parsed, &["Value"], &["QueryKind", "OnEmpty"], "StorageValue", args_span)?;

            StorageGenerics::Value {
                value: parsed.remove("Value").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                query_kind: parsed.remove("QueryKind").map(|binding| binding.ty),
                on_empty: parsed.remove("OnEmpty").map(|binding| binding.ty),
            }
        }
        StorageKind::Map => {
            check_generics(&parsed, &map_mandatory_generics, &map_optional_generics, "StorageMap", args_span)?;
            ...
            StorageGenerics::Map {
                ...
                key: parsed.remove("Key").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                value: parsed.remove("Value").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                query_kind: parsed.remove("QueryKind").map(|binding| binding.ty),
                on_empty: parsed.remove("OnEmpty").map(|binding| binding.ty),
        ...
        StorageKind::CountedMap => {
            check_generics(&parsed, &map_mandatory_generics, &map_optional_generics, "CountedStorageMap", args_span)?;
        ...
        StorageKind::DoubleMap => {
            ...
            StorageGenerics::DoubleMap {
                hasher1: parsed
                    ...
                    .map(|binding| binding.ty)
                    .unwrap_or(syn::parse_quote!(Blake2_128Concat)),
                key1: parsed.remove("Key1").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                hasher2: parsed
                    .remove("Hasher2")
                    .map(|binding| binding.ty)
                    .unwrap_or(syn::parse_quote!(Blake2_128Concat)),
                key2: parsed.remove("Key2").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                value: parsed.remove("Value").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                query_kind: parsed.remove("QueryKind").map(|binding| binding.ty),
                on_empty: parsed.remove("OnEmpty").map(|binding| binding.ty),
                max_values: parsed.remove("MaxValues").map(|binding| binding.ty),
        ...
        StorageKind::NMap => {
            ...
            StorageGenerics::NMap {
                keygen: parsed.remove("Key").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                value: parsed.remove("Value").map(|binding| binding.ty).expect("checked above as mandatory generic"),
                query_kind: parsed.remove("QueryKind").map(|binding| binding.ty),
                on_empty: parsed.remove("OnEmpty").map(|binding| binding.ty),
                max_values: parsed.remove("MaxValues").map(|binding| binding.ty),
...
fn process_unnamed_generics(
    ...
    let retrieve_arg = |arg_pos| {
        args.get(arg_pos).cloned().ok_or_else(|| {
            let msg = format!(
                "Invalid pallet::storage, unexpected number of generic argument, expect at least {} args, found {}.",
                arg_pos + 1,
                args.len(),
            );
    ...
    syn::parse2::<syn::Token![_]>(prefix_arg.to_token_stream()).map_err(|e| {
        let msg = "Invalid pallet::storage, for unnamed generic arguments the type first generic argument must be \
                   `_`, the argument is then replaced by macro.";
    ...
    let res = match storage {
        StorageKind::Value => (None, Metadata::Value { value: retrieve_arg(1)? }, retrieve_arg(2).ok(), false),
        StorageKind::Map => (
            None,
        ...
        StorageKind::DoubleMap => (
            None,
            Metadata::DoubleMap { key1: retrieve_arg(2)?, key2: retrieve_arg(4)?, value: retrieve_arg(5)? },
            retrieve_arg(6).ok(),
...
fn process_generics(
    ...
    let storage_kind = match &*segment.ident.to_string() {
        ...
        found => {
            let msg = format!(
                "Invalid pallet::storage, expected ident: `StorageValue` or `StorageMap` or `CountedStorageMap` or \
                 `StorageDoubleMap` or `StorageNMap` in order to expand metadata, found `{}`.",
                found,
            );
    ...
    let args = match &segment.arguments {
        ...
        _ => {
            let msg = "Invalid pallet::storage, invalid number of generic generic arguments, expect more that 0 \
                       generic arguments.";
    ...
    if args.args.iter().all(|gen| matches!(gen, syn::GenericArgument::Type(_))) {
        ...
        let msg = "Invalid pallet::storage, invalid generic declaration for storage. Expect only type generics or \
                   binding generics, e.g. `<Name1 = Gen1, Name2 = Gen2, ..>` or `<Gen1, Gen2, ..>`.";
...
fn extract_key(ty: &syn::Type) -> syn::Result<syn::Type> {
    ...
    if ty_params.args.len() != 2 {
        let msg = format!(
            "Invalid pallet::storage, unexpected number of generic arguments for Key struct, expected 2 args, found {}",
            ty_params.args.len()
        );
...
impl StorageDef {
    ...
    pub fn prefix(&self) -> String {
        self.rename_as.as_ref().map(syn::LitStr::value).unwrap_or_else(|| self.ident.to_string())
    }
    ...
    pub fn prefix_span(&self) -> proc_macro2::Span {
        self.rename_as.as_ref().map(syn::LitStr::span).unwrap_or_else(|| self.ident.span())
    }
    ...
    pub fn try_from(
        ...
        let query_kind = query_kind
            .map(|query_kind| {
                ...
                let result_query = match query_kind {
                    Type::Path(path) if path.path.segments.last().map_or(false, |s| s.ident == "OptionQuery") => {
                        return Ok(Some(QueryKind::OptionQuery));
                    }
                    Type::Path(TypePath { path: Path { segments, .. }, .. })
                        if segments.last().map_or(false, |s| s.ident == "ResultQuery") =>
                    {
                        segments.last().expect("segments is checked to have the last value; qed").clone()
                    }
                    Type::Path(path) if path.path.segments.last().map_or(false, |s| s.ident == "ValueQuery") => {
                ...
                let error_type = match result_query.arguments {
                    PathArguments::AngleBracketed(AngleBracketedGenericArguments { args, .. }) => {
                        if args.len() != 1 {
                            let msg = format!(
                                "Invalid pallet::storage, unexpected number of generic arguments for ResultQuery, \
                                 expected 1 type argument, found {}",
                                args.len(),
                            );
                    ...
                    args => {
                        let msg = format!(
                            "Invalid pallet::storage, unexpected generic args for ResultQuery, expected \
                             angle-bracketed arguments, found `{}`",
                ...
                match error_type {
                    GenericArgument::Type(Type::Path(TypePath {
                        ...
                        if err_variant.len() < 2 {
                            let msg = format!(
                                "Invalid pallet::storage, unexpected number of path segments for the generics in \
                                 ResultQuery, expected a path with at least 2 segments, found {}",
                                err_variant.len(),
                            );
                            return Err(syn::Error::new(err_variant.span(), msg));
                        }
                        let mut error = err_variant.clone();
                        let err_variant = error.pop().expect("Checked to have at least 2; qed").into_value().ident;
                        ...
                        Ok(Some(QueryKind::ResultQuery(syn::Path { leading_colon, segments: error }, err_variant)))
                    }
                    gen_arg => {
                        let msg = format!(
                            "Invalid pallet::storage, unexpected generic argument kind, expected a type path to a \
                             `PalletError` enum variant, found `{}`",
        ...
        if let (None, Some(getter)) = (query_kind.as_ref(), getter.as_ref()) {
            let msg = "Invalid pallet::storage, cannot generate getter because QueryKind is not identifiable. \
                       QueryKind must be `OptionQuery`, `ResultQuery`, `ValueQuery`, or default one to be \
                       identifiable.";
