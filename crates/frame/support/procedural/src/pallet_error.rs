    ...
    let max_encoded_size = match data {
        syn::Data::Struct(syn::DataStruct { fields, .. }) => match fields {
            ...
            | syn::Fields::Unnamed(syn::FieldsUnnamed { unnamed: fields, .. }) => {
                let maybe_field_tys =
                    fields.iter().map(|f| generate_field_types(f, &frame_support)).collect::<syn::Result<Vec<_>>>();
...
fn generate_field_types(field: &syn::Field, scrate: &syn::Ident) -> syn::Result<Option<proc_macro2::TokenStream>> {
...
fn generate_variant_field_types(
    ...
    match &variant.fields {
        ...
        | syn::Fields::Unnamed(syn::FieldsUnnamed { unnamed: fields, .. }) => {
            let field_tys =
                fields.iter().map(|field| generate_field_types(field, scrate)).collect::<syn::Result<Vec<_>>>()?;
