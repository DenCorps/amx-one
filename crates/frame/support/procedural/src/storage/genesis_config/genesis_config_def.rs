...
impl GenesisConfigDef {
    pub fn from_def(def: &DeclStorageDefExt) -> syn::Result<Self> {
        ...
        let is_generic = fields.iter().any(|field| ext::type_contains_ident(&field.typ, &def.module_runtime_generic));
        ...
        Ok(Self { is_generic, fields, genesis_struct_decl, genesis_struct, genesis_impl, genesis_where_clause })
    }
    ...
    fn get_genesis_config_field_defs(def: &DeclStorageDefExt) -> syn::Result<Vec<GenesisConfigFieldDef>> {
            ...
            .filter_map(|line| line.config.as_ref().map(|config_field| (config_field.clone(), line)))
        {
            ...
            let default = line
                .default_value
                .as_ref()
                .map(|d| if line.is_option { quote!( #d.unwrap_or_default() ) } else { quote!( #d ) })
        ...
        for line in &def.extra_genesis_config_lines {
            ...
            let default = line.default.as_ref().map(|e| quote!( #e )).unwrap_or_else(|| quote!(Default::default()));
