    ...
    ($struct:ident => $token:path) => {
        impl syn::parse::Parse for Opt<$struct> {
            fn parse(input: syn::parse::ParseStream) -> syn::parse::Result<Self> {
                if input.peek($token) { input.parse().map(|p| Opt { inner: Some(p) }) } else { Ok(Opt { inner: None }) }
            }
        }
    };
}
...
fn get_module_instance(
    ...
    if instantiable.as_ref().map_or(false, |i| i != "Instance") {
        let msg = format!(
            "Instance trait must be named `Instance`, other names are no longer supported, because it is now defined \
             at frame_support::traits::Instance. Expect `Instance` found `{}`",
    ...
    match (instance, instantiable, default_instance) {
        ...
        (None, Some(instantiable), _) => Err(syn::Error::new(
            instantiable.span(),
            format!("Expect instance generic for bound instantiable: {}. {}", instantiable, right_syntax,),
        )),
        (None, _, Some(default_instance)) => Err(syn::Error::new(
            default_instance.span(),
            format!("Expect instance generic for default instance: {}. {}", default_instance, right_syntax,),
        )),
    }
}
...
pub fn parse(input: syn::parse::ParseStream) -> syn::Result<super::DeclStorageDef> {
    ...
    for line in def.extra_genesis.inner.into_iter().flat_map(|o| o.content.content.lines.inner.into_iter()) {
        match line {
            ...
            AddExtraGenesisLineEnum::AddExtraGenesisBuild(def) => {
                if extra_genesis_build.is_some() {
                    return Err(syn::Error::new(def.span(), "Only one build expression allowed for extra genesis"));
                }
...
fn parse_storage_line_defs(defs: impl Iterator<Item = DeclStorageLine>) -> syn::Result<Vec<super::StorageLineDef>> {
    ...
    for line in defs {
        ...
        let config = if let Some(config) = line.config.inner {
            if let Some(ident) = config.expr.content {
                ...
                return Err(syn::Error::new(
                    config.span(),
                    "Invalid storage definition, couldn't find config identifier: storage must either have a get \
        ...
        if let Some(ref config) = config {
            storage_lines.iter().filter_map(|sl| sl.config.as_ref()).try_for_each(|other_config| {
                if other_config == config {
                    Err(syn::Error::new(config.span(), "`config()`/`get()` with the same name already defined."))
                } else {
                    Ok(())
                }
            })?;
        ...
        let no_hasher_error = || {
            syn::Error::new(span, "Default hasher has been removed, use explicit hasher(blake2_128_concat) instead.")
        };
