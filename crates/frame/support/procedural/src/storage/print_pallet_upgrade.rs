...
pub fn maybe_print_pallet_upgrade(def: &super::DeclStorageDefExt) {
    ...
    let config_gen = if def.optional_instance.is_some() { "<I: 'static = ()>" } else { Default::default() };

    let impl_gen = if def.optional_instance.is_some() { "<T: Config<I>, I: 'static>" } else { "<T: Config>" };

    let decl_gen = if def.optional_instance.is_some() { "<T, I=()>" } else { "<T>" };

    let full_decl_gen = if def.optional_instance.is_some() { "<T: Config<I>, I: 'static = ()>" } else { "<T: Config>" };
    ...
    if !genesis_config_builder_def.blocks.is_empty() {
        ...
        let genesis_config_impl_gen = if genesis_config_def.is_generic { impl_gen } else { Default::default() };

        let genesis_config_use_gen = if genesis_config_def.is_generic { use_gen } else { Default::default() };
	...
	{{
        ...
        let genesis_config_build = genesis_config_builder_def.blocks.iter().fold(String::new(), |res, block| {
            format!(
                "{}
					{}",
                res,
...
",
        ...
        let comma_query_kind = if line.is_option {
            if line.default_value.is_some() { ", OptionQuery" } else { Default::default() }
        } else {
            ", ValueQuery"
        };

        let comma_default_value_getter_name =
            line.default_value.as_ref().map(|_| format!(", DefaultFor{}", line.name)).unwrap_or_else(String::new);

        let typ = match &line.storage_type {
            StorageLineTypeDef::Map(map) => {
                format!(
                    "StorageMap<_, {hasher}, {key}, {value_type}{comma_query_kind}{comma_default_value_getter_name}>",
            ...
            StorageLineTypeDef::DoubleMap(double_map) => {
                format!(
                    "StorageDoubleMap<_, {hasher1}, {key1}, {hasher2}, {key2}, \
                     {value_type}{comma_query_kind}{comma_default_value_getter_name}>",
            ...
            StorageLineTypeDef::NMap(map) => {
                format!(
                    "StorageNMap<_, {keygen}, {value_type}{comma_query_kind}{comma_default_value_getter_name}>",
            ...
            StorageLineTypeDef::Simple(_) => {
                format!(
                    "StorageValue<_, {value_type}{comma_query_kind}{comma_default_value_getter_name}>",
                    value_type = to_cleaned_string(&value_type),
                    comma_query_kind = comma_query_kind,
                    comma_default_value_getter_name = comma_default_value_getter_name,
                )
        ...
        let additional_comment = if line.is_option && line.default_value.is_some() {
            " // TODO: This type of storage is no longer supported: `OptionQuery` cannot be used alongside a not-none \
             value on empty storage. Please use `ValueQuery` instead."
        } else {
            ""
        };
