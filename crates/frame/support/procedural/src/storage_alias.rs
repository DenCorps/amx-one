...
use syn::{parenthesized, token, Attribute, Error, Ident, Result, Token, Type, TypeParam, Visibility, WhereClause};
...
impl SimplePath {
    ...
    fn get_ident(&self) -> Option<&Ident> {
        if self.segments.len() != 1 || self.leading_colon.is_some() { None } else { self.segments.first() }
    }
}
...
impl StorageType {
    ...
    fn generate_type_declaration(
        ...
        match self {
            ...
            Self::CountedMap { value_ty, query_type, hasher_ty, key_ty, prefix_generics, .. }
            ...
            Self::DoubleMap {
                value_ty, query_type, hasher1_ty, key1_ty, hasher2_ty, key2_ty, prefix_generics, ..
            } => {
                let query_type = query_type.as_ref().map(|(c, t)| quote!(#c #t));
...
fn generate_storage_instance(
    ...
    let (pallet_prefix, impl_generics, type_generics) = if let Some((prefix_generics, storage_generics)) =
        prefix_generics.and_then(|p| storage_generics.map(|s| (p, s)))
    {
        ...
        let impl_generics =
            storage_generics.impl_generics().filter(|g| prefix_generics.params.iter().any(|pg| *pg == g.ident));
