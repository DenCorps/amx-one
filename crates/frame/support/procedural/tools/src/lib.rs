...
// FIXME #1569, remove the following functions, which are copied from mp-api-macros
use proc_macro2::{Span, TokenStream};
use syn::Ident;

fn generate_hidden_includes_mod_name(unique_id: &str) -> Ident {
    Ident::new(&format!("mp_api_hidden_includes_{}", unique_id), Span::call_site())
}
    ...
    attrs
        ...
        .filter_map(|attr| {
            if let syn::Meta::NameValue(meta) = &attr.meta {
                meta.path.get_ident().filter(|ident| *ident == "doc").map(|_| meta.value.clone())
            } else {
                None
            }
        })
