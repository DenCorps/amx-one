...
use mp_runtime::generic::{CheckedExtrinsic, UncheckedExtrinsic};
pub use mp_runtime::traits::Dispatchable;
use mp_runtime::traits::SignedExtension;
pub use mp_runtime::transaction_validity::TransactionPriority;
pub use mp_runtime::{DispatchError, RuntimeDebug};
#[cfg(feature = "std")]
use serde::{Deserialize, Serialize};
pub use sp_weights::Weight;

pub use crate::codec::{Codec, Decode, Encode, EncodeAsRef, EncodeLike, HasCompact, Input, MaxEncodedLen, Output};
pub use crate::scale_info::TypeInfo;
pub use crate::sp_std::prelude::{Clone, Eq, PartialEq, Vec};
pub use crate::sp_std::{fmt, marker, result};
pub use crate::traits::{
    CallMetadata, GetCallIndex, GetCallMetadata, GetCallName, GetStorageVersion, UnfilteredDispatchable,
};

/// The return type of a `Dispatchable` in frame. When returned explicitly from
/// a dispatchable function it allows overriding the default `PostDispatchInfo`
/// returned from a dispatch.
pub type DispatchResultWithPostInfo = mp_runtime::DispatchResultWithInfo<PostDispatchInfo>;
...
pub type DispatchResult = Result<(), mp_runtime::DispatchError>;

/// The error type contained in a `DispatchResultWithPostInfo`.
pub type DispatchErrorWithPostInfo = mp_runtime::DispatchErrorWithPostInfo<PostDispatchInfo>;
...
impl PostDispatchInfo {
    ...
    pub fn calc_actual_weight(&self, info: &DispatchInfo) -> Weight {
        if let Some(actual_weight) = self.actual_weight { actual_weight.min(info.weight) } else { info.weight }
    }
...
impl mp_runtime::traits::Printable for PostDispatchInfo {
    fn print(&self) {
        "actual_weight=".print();
    ...
    T: Into<DispatchError>,
{
    fn with_weight(self, actual_weight: Weight) -> DispatchErrorWithPostInfo {
        DispatchErrorWithPostInfo {
            post_info: PostDispatchInfo { actual_weight: Some(actual_weight), pays_fee: Default::default() },
            error: self.into(),
        }
    }
}
...
#[cfg(feature = "std")]
impl<Call: Encode + GetDispatchInfo, Extra: Encode> GetDispatchInfo for mp_runtime::testing::TestXt<Call, Extra> {
    fn get_dispatch_info(&self) -> DispatchInfo {
...
/// implement the [`OnRuntimeUpgrade`](../mp_runtime/traits/trait.OnRuntimeUpgrade.html) trait.
...
macro_rules! decl_module {
	...
	(@impl_try_state_default
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			...
			) -> Result<(), $crate::mp_runtime::TryRuntimeError> {
				let pallet_name = <<
					$trait_instance
					as
					$system::Config
	...
	(@impl_on_runtime_upgrade
			...
			for $module<$trait_instance$(, $instance)?> where $( $other_where_bounds )*
		{
			...
			#[cfg(feature = "try-runtime")]
			fn pre_upgrade() -> Result<$crate::sp_std::vec::Vec<u8>, $crate::mp_runtime::TryRuntimeError> {
				Ok($crate::sp_std::vec::Vec::new())
			}

			#[cfg(feature = "try-runtime")]
			fn post_upgrade(_: $crate::sp_std::vec::Vec<u8>) -> Result<(), $crate::mp_runtime::TryRuntimeError> {
				Ok(())
			}
		}
	};
...
mod tests {
    ...
    use super::*;
    use crate::dispatch::{DispatchClass, DispatchInfo, Pays};
    use crate::metadata_ir::*;
    use crate::traits::{
        CallerTrait, CrateVersion, Get, GetCallName, IntegrityTest, OnFinalize, OnIdle, OnInitialize, OnRuntimeUpgrade,
        PalletInfo,
    };
    ...
    #[derive(TypeInfo, crate::RuntimeDebug, Eq, PartialEq, Clone, Encode, Decode, MaxEncodedLen)]
    ...
    fn module_json_metadata() {
        ...
        let expected_metadata = PalletCallMetadataIR { ty: scale_info::meta_type::<Call<TraitImpl>>() };
    ...
    fn on_initialize_should_work_2() {
        assert_eq!(<Module<TraitImpl> as OnInitialize<u32>>::on_initialize(10), Weight::from_parts(7, 0));
    }
    ...
    fn on_runtime_upgrade_should_work() {
        sp_io::TestExternalities::default().execute_with(|| {
            assert_eq!(<Module<TraitImpl> as OnRuntimeUpgrade>::on_runtime_upgrade(), Weight::from_parts(10, 0))
        });
    }
    ...
    fn weight_should_attach_to_call_enum() {
        ...
        assert_eq!(
            ...
            DispatchInfo { weight: Weight::from_parts(5, 0), class: DispatchClass::Operational, pays_fee: Pays::Yes },
        );
        // custom basic
        assert_eq!(
            Call::<TraitImpl>::aux_3 {}.get_dispatch_info(),
            DispatchInfo { weight: Weight::from_parts(3, 0), class: DispatchClass::Normal, pays_fee: Pays::Yes },
        );
    }
    ...
    pub fn from_actual_ref_time(ref_time: Option<u64>) -> PostDispatchInfo {
        PostDispatchInfo { actual_weight: ref_time.map(|t| Weight::from_all(t)), pays_fee: Default::default() }
    }
...
mod weight_tests {
    ...
    fn extract_actual_weight_works() {
        ...
        assert_eq!(extract_actual_weight(&Ok(from_actual_ref_time(Some(7))), &pre), Weight::from_parts(7, 0));
        assert_eq!(extract_actual_weight(&Ok(from_actual_ref_time(Some(1000))), &pre), Weight::from_parts(1000, 0));
        assert_eq!(
            extract_actual_weight(&Err(DispatchError::BadOrigin.with_weight(Weight::from_parts(9, 0))), &pre),
    ...
    fn extract_actual_weight_caps_at_pre_weight() {
        let pre = DispatchInfo { weight: Weight::from_parts(1000, 0), ..Default::default() };
        assert_eq!(extract_actual_weight(&Ok(from_actual_ref_time(Some(1250))), &pre), Weight::from_parts(1000, 0));
        assert_eq!(
            extract_actual_weight(&Err(DispatchError::BadOrigin.with_weight(Weight::from_parts(1300, 0))), &pre),
    ...
    fn extract_actual_pays_fee_works() {
        ...
        assert_eq!(extract_actual_pays_fee(&Ok(from_actual_ref_time(Some(1000)).into()), &pre), Pays::Yes);
        assert_eq!(extract_actual_pays_fee(&Ok(from_post_weight_info(Some(1000), Pays::Yes)), &pre), Pays::Yes);
        assert_eq!(extract_actual_pays_fee(&Ok(from_post_weight_info(Some(1000), Pays::No)), &pre), Pays::No);
        assert_eq!(
            extract_actual_pays_fee(&Err(DispatchError::BadOrigin.with_weight(Weight::from_parts(9, 0))), &pre),
            Pays::Yes
        );
        assert_eq!(
        ...
        let pre = DispatchInfo { weight: Weight::from_parts(1000, 0), pays_fee: Pays::No, ..Default::default() };
        ...
        assert_eq!(extract_actual_pays_fee(&Ok(from_post_weight_info(Some(1000), Pays::Yes)), &pre), Pays::No);
    }
}

#[cfg(test)]
mod per_dispatch_class_tests {
    use mp_runtime::traits::Zero;
    use DispatchClass::*;
    ...
    #[test]
    fn add_works() {
        let a = PerDispatchClass { normal: (5, 10).into(), operational: (20, 30).into(), mandatory: Weight::MAX };
        assert_eq!(
            a.clone()
                .add((20, 5).into(), Normal)
                .add((10, 10).into(), Operational)
                .add((u64::MAX, 3).into(), Mandatory),
            PerDispatchClass { normal: (25, 15).into(), operational: (30, 40).into(), mandatory: Weight::MAX }
        );
        let b = a.add(Weight::MAX, Normal).add(Weight::MAX, Operational).add(Weight::MAX, Mandatory);
        assert_eq!(b, PerDispatchClass { normal: Weight::MAX, operational: Weight::MAX, mandatory: Weight::MAX });
    ...
    fn checked_accrue_works() {
        ...
        assert_eq!(
            a,
            PerDispatchClass { normal: (10, 2).into(), operational: (10, 12).into(), mandatory: (5, 6).into() }
        );
        ...
        assert_eq!(a, PerDispatchClass { normal: Weight::MAX, operational: (10, 12).into(), mandatory: (5, 6).into() });
    }

    #[test]
    fn checked_accrue_does_not_modify_on_error() {
        let mut a =
            PerDispatchClass { normal: 0.into(), operational: Weight::MAX / 2 + 2.into(), mandatory: 10.into() };
        ...
        assert_eq!(
            a,
            PerDispatchClass { normal: Weight::MAX, operational: Weight::MAX / 2 + 2.into(), mandatory: 10.into() }
        );
    }
    ...
    fn total_works() {
        ...
        assert_eq!(
            PerDispatchClass { normal: 0.into(), operational: (10, 20).into(), mandatory: (20, u64::MAX).into() }
                .total(),
            (30, u64::MAX).into()
        );
