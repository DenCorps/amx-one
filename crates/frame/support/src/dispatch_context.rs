...
pub fn with_context<T: 'static, R>(callback: impl FnOnce(&mut Value<T>) -> R) -> Option<R> {
    DISPATCH_CONTEXT::with(|c| match c.entry(TypeId::of::<T>()) {
        Entry::Occupied(mut o) => {
            ...
            if value.is_none() {
                log::error!("Failed to downcast value for type {} in dispatch context!", sp_std::any::type_name::<T>(),);
            }
...
mod tests {
    ...
    fn dispatch_context_works() {
        ...
        let res = run_in_context(|| {
            ...
            run_in_context(|| {
                run_in_context(|| run_in_context(|| with_context::<Context, _>(|v| v.or_default().0).unwrap()))
            })
        });
