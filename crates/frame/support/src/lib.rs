...
#[doc(hidden)]
pub use log;
pub use mp_api::metadata_ir;
#[cfg(feature = "std")]
#[doc(hidden)]
pub use mp_runtime::{bounded_btree_map, bounded_vec};
#[doc(hidden)]
pub use mp_runtime::{RuntimeDebug, StateVersion};
#[cfg(feature = "std")]
#[doc(hidden)]
pub use once_cell;
#[doc(hidden)]
...
pub mod unsigned {
    #[doc(hidden)]
    pub use crate::mp_runtime::traits::ValidateUnsigned;
    #[doc(hidden)]
    pub use crate::mp_runtime::transaction_validity::{
        TransactionSource, TransactionValidity, TransactionValidityError, UnknownTransaction,
    };
}

use codec::{Decode, Encode};
pub use mp_runtime::traits::Printable;
use mp_runtime::TypeId;
pub use mp_runtime::{self, print, ConsensusEngineId, MAX_MODULE_ERROR_ENCODED_SIZE};
...
pub use self::storage::bounded_btree_map::BoundedBTreeMap;
pub use self::storage::bounded_btree_set::BoundedBTreeSet;
pub use self::storage::bounded_vec::{BoundedSlice, BoundedVec};
...
pub use self::storage::{
    migration, IterableStorageDoubleMap, IterableStorageMap, IterableStorageNMap, StorageDoubleMap, StorageMap,
    StorageNMap, StoragePrefixedMap, StorageValue,
};
...
/// use frame_support::{codec, Twox64Concat};
...
/// // to `Vec<u8>`
/// #[storage_alias]
/// type OtherStorageName =
///     StorageDoubleMap<OtherPrefix, Twox64Concat, u32, Twox64Concat, u32, Vec<u8>>;
///
...
/// #[storage_alias]
/// type SomeStorageName = StorageMap<Prefix, Twox64Concat, u32, Vec<u8>, ValueQuery>;
///
/// // generate a map from `Config::AccountId` (with hasher `Twox64Concat`) to `Vec<u8>`
/// trait Config {
///     type AccountId: codec::FullCodec;
/// }
/// #[storage_alias]
...
/// #[storage_alias]
/// type SomeNMap =
///     StorageNMap<Prefix, (NMapKey<Twox64Concat, u32>, NMapKey<Twox64Concat, u64>), Vec<u8>>;
///
...
pub use frame_support_procedural::{
    construct_runtime, decl_storage, match_and_insert, transactional, PalletError, RuntimeDebugNoBound,
};
...
pub mod tests {
    ...
    use mp_runtime::{generic, BuildStorage};
    ...
    use crate::metadata_ir::{
        PalletStorageMetadataIR, StorageEntryMetadataIR, StorageEntryModifierIR, StorageEntryTypeIR, StorageHasherIR,
    };
    ...
    pub mod frame_system {
        ...
        pub type GenericData<T: Config> = StorageMap<_, Identity, T::BlockNumber, T::BlockNumber, ValueQuery>;

        #[pallet::storage]
        #[pallet::getter(fn generic_data2)]
        pub type GenericData2<T: Config> = StorageMap<_, Blake2_128Concat, T::BlockNumber, T::BlockNumber, OptionQuery>;

        #[pallet::storage]
        pub type DataDM<T> = StorageDoubleMap<_, Twox64Concat, u32, Blake2_128Concat, u32, u64, ValueQuery>;

        #[pallet::storage]
        pub type GenericDataDM<T: Config> =
            StorageDoubleMap<_, Blake2_128Concat, T::BlockNumber, Identity, T::BlockNumber, T::BlockNumber, ValueQuery>;
        ...
        pub type AppendableDM<T: Config> =
            StorageDoubleMap<_, Blake2_128Concat, u32, Blake2_128Concat, T::BlockNumber, Vec<u32>, ValueQuery>;
    ...
    fn storage_alias_works() {
        new_test_ext().execute_with(|| {
            #[crate::storage_alias]
            type GenericData2<T> =
                StorageMap<System, Blake2_128Concat, <T as Config>::BlockNumber, <T as Config>::BlockNumber>;
            ...
            pub type GenericData<T> =
                StorageMap<Test2, Blake2_128Concat, <T as Config>::BlockNumber, <T as Config>::BlockNumber>;
        });
    }
    ...
    fn double_map_swap_works() {
        new_test_ext().execute_with(|| {
            ...
            let get_all =
                || vec![DataDM::get(0, 1), DataDM::get(1, 0), DataDM::get(1, 1), DataDM::get(2, 0), DataDM::get(2, 1)];
        ...
        PalletStorageMetadataIR {
            ...
            entries: vec![
                ...
                StorageEntryMetadataIR {
                    ...
                    modifier: StorageEntryModifierIR::Default,
                    ty: StorageEntryTypeIR::Map {
                        hashers: vec![StorageHasherIR::Blake2_128Concat, StorageHasherIR::Blake2_128Concat],
                        key: scale_info::meta_type::<(u32, u32)>(),
...
pub mod pallet_prelude {
    ...
    pub use mp_runtime::traits::{MaybeSerializeDeserialize, Member, ValidateUnsigned};
    pub use mp_runtime::transaction_validity::{
        InvalidTransaction, TransactionLongevity, TransactionPriority, TransactionSource, TransactionTag,
        TransactionValidity, TransactionValidityError, UnknownTransaction, ValidTransaction,
    };
    pub use mp_runtime::MAX_MODULE_ERROR_ENCODED_SIZE;
    pub use scale_info::TypeInfo;
    pub use sp_std::marker::PhantomData;
    pub use sp_weights::Weight;

    pub use crate::dispatch::{
        DispatchClass, DispatchError, DispatchResult, DispatchResultWithPostInfo, Parameter, Pays,
    };
    ...
    pub use crate::storage::types::{
        CountedStorageMap, Key as NMapKey, OptionQuery, ResultQuery, StorageDoubleMap, StorageMap, StorageNMap,
        StorageValue, ValueQuery,
    };
    #[cfg(feature = "std")]
    pub use crate::traits::GenesisBuild;
    pub use crate::traits::{
        ConstU32, EnsureOrigin, Get, GetDefault, GetStorageVersion, Hooks, IsType, PalletInfoAccess, StorageInfoTrait,
        StorageVersion, TypedGet,
    };
    pub use crate::{
        ensure, storage, Blake2_128, Blake2_128Concat, Blake2_256, CloneNoBound, DebugNoBound, EqNoBound, Identity,
...
/// argument cannot be specified anywhere else, including but not limited to the `#[pallet::pallet]`
/// attribute macro.
///
...
/// `RuntimeEvent` exists as a config item in your `#[pallet::config]`.
///
...
/// such as [`Option`] and [`PhantomData`](`frame_support::dispatch::marker::PhantomData`),
/// and hence in most use cases, a manual implementation is not necessary and is discouraged.
///
...
/// * [`StorageValue`](`pallet_prelude::StorageValue`) expects `Value` and optionally `QueryKind`
///   and `OnEmpty`,
/// * [`StorageMap`](`pallet_prelude::StorageMap`) expects `Hasher`, `Key`, `Value` and optionally
///   `QueryKind` and `OnEmpty`,
/// * [`CountedStorageMap`](`pallet_prelude::CountedStorageMap`) expects `Hasher`, `Key`, `Value`
///   and optionally `QueryKind` and `OnEmpty`,
/// * [`StorageDoubleMap`](`pallet_prelude::StorageDoubleMap`) expects `Hasher1`, `Key1`, `Hasher2`,
///   `Key2`, `Value` and optionally `QueryKind` and `OnEmpty`.
///
...
/// 2. [`ValueQuery`](`frame_support::storage::types::ValueQuery`) causes storage map APIs
/// that    return a `QueryKind` to instead return the value type. In cases where a value does not
...
/// optional where clause.
///
/// NOTE: There is also the [`mp_runtime::traits::SignedExtension`] trait that can be used to
...
///
/// 	#[derive(codec::Encode, mp_runtime::RuntimeDebug)]
...
///     - `#[compact]` must now be written [`#[pallet::compact]`](#palletcompact-some_arg-some_type)
...
///     - for storages with value being `Option<$something>` make generic `Value` being `$something`
...
///     - for storages with default value: `= $expr;` provide some specific `OnEmpty` generic. To do
///       so use of `#[pallet::type_value]` to generate the wanted struct to put. example:
...
///
/// * compare metadata. Use [subsee](https://github.com/ascjones/subsee) to fetch the metadata and
///   do a diff of the resulting json before and after migration. This checks for:
...
///     * [`ValidateUnsigned`](`pallet_prelude::ValidateUnsigned`) was moved inside the macro under
/// 	  [`#[pallet::validate_unsigned)]`](#validate-unsigned-palletvalidate_unsigned-optional)
/// 	  if it exists
///     * [`ProvideInherent`](`pallet_prelude::ProvideInherent`) was moved inside the macro under
///       [`#[pallet::inherent)]`](#inherent-palletinherent-optional) if it exists
///     * `on_initialize` / `on_finalize` / `on_runtime_upgrade` / `offchain_worker` were moved to
///       the `Hooks` implementation
///     * storages with `config(..)` were converted to `GenesisConfig` field, and their default is
///       `= $expr;` if the storage has a default value
...
///   `pallet_prefix` given as input of `decl_storage` with the syntax `as Example`). Thus a runtime
///   using the pallet must be careful with this change. To handle this change:
///     * either ensure that the name of the pallet given to `construct_runtime!` is the same as the
///       name the pallet was giving to `decl_storage`,
...
pub mod pallet_macros {
    pub use frame_support_procedural::{
        call_index, compact, composite_enum, config, constant, disable_frame_system_supertrait_check, error, event,
        extra_constants, generate_deposit, generate_store, genesis_build, genesis_config, getter, hooks, inherent,
        origin, storage, storage_prefix, storage_version, type_value, unbounded, validate_unsigned, weight,
        whitelist_storage,
    };
}
