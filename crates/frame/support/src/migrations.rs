...
use crate::traits::{GetStorageVersion, NoStorageVersionSet, PalletInfoAccess, StorageVersion};
use crate::weights::{RuntimeDbWeight, Weight};
...
impl<T: GetStorageVersion<CurrentStorageVersion = NoStorageVersionSet> + PalletInfoAccess> StoreCurrentStorageVersion<T>
    for NoStorageVersionSet
{
    fn store_current_storage_version() {
...
pub struct RemovePallet<P: Get<&'static str>, DbWeight: Get<RuntimeDbWeight>>(PhantomData<(P, DbWeight)>);
    ...
    for RemovePallet<P, DbWeight>
{
    ...
    #[cfg(feature = "try-runtime")]
    fn pre_upgrade() -> Result<Vec<u8>, mp_runtime::TryRuntimeError> {
        let hashed_prefix = twox_128(P::get().as_bytes());
    ...
    #[cfg(feature = "try-runtime")]
    fn post_upgrade(_state: Vec<u8>) -> Result<(), mp_runtime::TryRuntimeError> {
        let hashed_prefix = twox_128(P::get().as_bytes());
        match contains_prefixed_key(&hashed_prefix) {
            true => {
