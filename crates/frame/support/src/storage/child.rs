...
pub fn exists(child_info: &ChildInfo, key: &[u8]) -> bool {
    match child_info.child_type() {
        ChildType::ParentKeyId => sp_io::default_child_storage::exists(child_info.storage_key(), key),
    }
}
...
pub fn kill_storage(child_info: &ChildInfo, limit: Option<u32>) -> KillStorageResult {
    match child_info.child_type() {
        ChildType::ParentKeyId => sp_io::default_child_storage::storage_kill(child_info.storage_key(), limit),
    }
}
...
pub fn clear_storage(
    ...
    let r = match child_info.child_type() {
        ChildType::ParentKeyId => sp_io::default_child_storage::storage_kill(child_info.storage_key(), maybe_limit),
    };
