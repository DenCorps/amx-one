...
use crate::storage::{self, storage_prefix, unhashed, KeyPrefixIterator, PrefixIterator, StorageAppend};
use crate::Never;
...
pub trait StorageDoubleMap<K1: FullEncode, K2: FullEncode, V: FullCodec> {
        ...
        KArg2: EncodeLike<K2>,
    {
        ...
        let mut final_key =
            Vec::with_capacity(storage_prefix.len() + key1_hashed.as_ref().len() + key2_hashed.as_ref().len());
    ...
    G: StorageDoubleMap<K1, K2, V>,
{
    ...
    where
        KArg1: EncodeLike<K1>,
    {
        unhashed::clear_prefix(Self::storage_double_map_final_key1(k1).as_ref(), maybe_limit, None).into()
    }
    ...
    where
        KArg1: EncodeLike<K1>,
    {
        unhashed::clear_prefix(Self::storage_double_map_final_key1(k1).as_ref(), Some(limit), maybe_cursor).into()
    }
...
mod test_iterators {
    ...
    use crate::storage::generator::tests::*;
    ...
    fn double_map_reversible_reversible_iteration() {
        sp_io::TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(DoubleMap::iter().collect::<Vec<_>>(), vec![(3, 3, 3), (0, 0, 0), (2, 2, 2), (1, 1, 1)],);

            assert_eq!(DoubleMap::iter_keys().collect::<Vec<_>>(), vec![(3, 3), (0, 0), (2, 2), (1, 1)],);

            assert_eq!(DoubleMap::iter_values().collect::<Vec<_>>(), vec![3, 0, 2, 1]);

            assert_eq!(DoubleMap::drain().collect::<Vec<_>>(), vec![(3, 3, 3), (0, 0, 0), (2, 2, 2), (1, 1, 1)],);
            ...
            assert_eq!(DoubleMap::iter_prefix(k1).collect::<Vec<_>>(), vec![(1, 1), (2, 2), (0, 0), (3, 3)],);
            ...
            assert_eq!(DoubleMap::drain_prefix(k1).collect::<Vec<_>>(), vec![(1, 1), (2, 2), (0, 0), (3, 3)],);
            ...
            // Wrong key2
            unhashed::put(&[prefix.clone(), crate::Blake2_128Concat::hash(&1u16.encode())].concat(), &3u64.encode());
            ...
            assert_eq!(DoubleMap::iter().collect::<Vec<_>>(), vec![(3, 3, 6), (0, 0, 0), (2, 2, 4), (1, 1, 2)],);
        })
    }
}
