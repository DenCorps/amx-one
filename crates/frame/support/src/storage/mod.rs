...
use mp_runtime::generic::{Digest, DigestItem};
pub use mp_runtime::TransactionOutcome;
...
pub use self::stream_iter::StorageStreamIter;
pub use self::transactional::{in_storage_layer, with_storage_layer, with_transaction, with_transaction_unchecked};
pub use self::types::StorageEntryMetadataBuilder;
use crate::hash::{ReversibleStorageHasher, StorageHasher};
use crate::storage::types::{
    EncodeLikeTuple, HasKeyPrefix, HasReversibleKeyPrefix, KeyGenerator, ReversibleKeyGenerator, TupleToEncodedIter,
};
...
pub trait StorageDoubleMap<K1: FullEncode, K2: FullEncode, V: FullCodec> {
    ...
    /// final call.
    fn clear_prefix<KArg1>(k1: KArg1, limit: u32, maybe_cursor: Option<&[u8]>) -> sp_io::MultiRemovalResults
    where
        KArg1: ?Sized + EncodeLike<K1>;
...
pub trait StorageNMap<K: KeyGenerator, V: FullCodec> {
    ...
    fn clear_prefix<KP>(partial_key: KP, limit: u32, maybe_cursor: Option<&[u8]>) -> sp_io::MultiRemovalResults
    where
        K: HasKeyPrefix<KP>;
...
impl<T, OnRemoval> PrefixIterator<T, OnRemoval> {
    ...
    /// undecoded value.
    pub fn new(prefix: Vec<u8>, previous_key: Vec<u8>, decode_fn: fn(&[u8], &[u8]) -> Result<T, codec::Error>) -> Self {
            ...
            break match maybe_next {
                Some(next) => {
                    ...
                    let item = match (self.closure)(raw_key_without_prefix, &raw_value[..]) {
                        Ok(item) => item,
                        Err(e) => {
                            log::error!("(key, value) failed to decode at {:?}: {:?}", self.previous_key, e,);
                            continue;
                        }
                    };
...
impl<T> KeyPrefixIterator<T> {
    ...
    /// current item.
    pub fn new(prefix: Vec<u8>, previous_key: Vec<u8>, decode_fn: fn(&[u8]) -> Result<T, codec::Error>) -> Self {
...
impl<T: Decode + Sized> ChildTriePrefixIterator<(Vec<u8>, T)> {
    ...
    pub fn with_prefix(child_info: &ChildInfo, prefix: &[u8]) -> Self {
        ...
        Self { prefix, child_info: child_info.clone(), previous_key, drain: false, fetch_previous_key: true, closure }
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            let maybe_next = if self.fetch_previous_key {
                ...
                sp_io::default_child_storage::next_key(self.child_info.storage_key(), &self.previous_key)
...
pub trait TryAppendMap<K: Encode, T: StorageTryAppend<I>, I: Encode> {
    ...
    ///
    /// This might fail if bounds are not respected.
    fn try_append<LikeK: EncodeLike<K> + Clone, LikeI: EncodeLike<I>>(key: LikeK, item: LikeI) -> Result<(), ()>;
}
...
mod test {
    ...
    use crate::pallet_prelude::NMapKey;
    ...
    fn child_trie_prefixed_map_works() {
        TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(
                ChildTriePrefixIterator::with_prefix(&child_info_a, &[2]).collect::<Vec<(Vec<u8>, u16)>>(),
                vec![(vec![], 8), (vec![2, 3], 8),],
            );

            assert_eq!(
                ChildTriePrefixIterator::with_prefix(&child_info_a, &[2]).drain().collect::<Vec<(Vec<u8>, u16)>>(),
                vec![(vec![], 8), (vec![2, 3], 8),],
            );

            // The only remaining is the ones outside prefix
            assert_eq!(
                ChildTriePrefixIterator::with_prefix(&child_info_a, &[]).collect::<Vec<(Vec<u8>, u8)>>(),
    ...
    #[crate::storage_alias]
    type FooDoubleMap = StorageDoubleMap<Prefix, Twox128, u32, Twox128, u32, BoundedVec<u32, ConstU32<7>>>;
    #[crate::storage_alias]
    type FooTripleMap = StorageNMap<Prefix, (NMapKey<Twox128, u32>, NMapKey<Twox128, u32>, NMapKey<Twox128, u32>), u64>;
    ...
    fn try_append_works() {
        ...
        TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(FooMap::get(2).unwrap(), BoundedVec::<u32, ConstU32<7>>::try_from(vec![4]).unwrap(),);
            assert_ok!(FooMap::try_append(2, 5));
            assert_eq!(FooMap::get(2).unwrap(), BoundedVec::<u32, ConstU32<7>>::try_from(vec![4, 5]).unwrap(),);
        });
