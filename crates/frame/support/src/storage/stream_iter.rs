...
mod private {
    ...
    impl<K: codec::Decode, V: codec::Decode> StreamIter for sp_std::collections::btree_map::BTreeMap<K, V> {
...
impl<T: private::StreamIter + codec::FullCodec, StorageValue: super::StorageValue<T>> StorageStreamIter<T>
    for StorageValue
{
    fn stream_iter() -> T::Iterator {
...
impl StorageInput {
    ...
    fn new(key: Vec<u8>) -> Self {
        ...
        let (total_length, exists) = if let Some(total_length) = sp_io::storage::read(&key, &mut buffer, 0) {
            (total_length, true)
        } else {
            (0, false)
        };
    ...
    fn fill_buffer(&mut self) -> Result<(), codec::Error> {
        ...
        {
            let bytes_read = sp_std::cmp::min(length_minus_offset as usize, self.buffer.len() - present_bytes);
    ...
    fn read_big_item(&mut self, into: &mut [u8]) -> Result<(), codec::Error> {
        ...
        if let Some(length_minus_offset) = sp_io::storage::read(&self.key, &mut out_remaining, self.offset) {
    ...
    fn ensure_total_length_did_not_change(&mut self, length_minus_offset: u32) -> Result<(), codec::Error> {
...
impl codec::Input for StorageInput {
    fn remaining_len(&mut self) -> Result<Option<usize>, codec::Error> {
        Ok(Some(
            self.total_length.saturating_sub(self.offset.saturating_sub((self.buffer.len() - self.buffer_pos) as u32))
                as usize,
        ))
    }
...
mod tests {
    ...
    fn remaining_len_works() {
        sp_io::TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(5 * std::mem::size_of::<u32>(), input.remaining_len().ok().flatten().unwrap());
            ...
            let total_data_len =
                data.iter().map(|v| v.len() + Compact::<u32>::compact_len(&(v.len() as u32)) as usize).sum::<usize>();
            assert_eq!(
    ...
    fn detects_value_total_length_change() {
        sp_io::TestExternalities::default().execute_with(|| {
            let test_data: Vec<Vec<Vec<u8>>> = vec![
                ...
                vec![vec![0; STORAGE_INPUT_BUFFER_CAPACITY - 1], vec![1; STORAGE_INPUT_BUFFER_CAPACITY - 1]],
            ];
    ...
    fn reading_big_intermediate_value() {
        sp_io::TestExternalities::default().execute_with(|| {
            let data: Vec<Vec<u8>> = vec![vec![0; 20], vec![1; STORAGE_INPUT_BUFFER_CAPACITY * 2], vec![2; 30]];
            TestVecVecU8::put(&data);
    ...
    fn reading_more_data_as_in_the_state_is_detected() {
        sp_io::TestExternalities::default().execute_with(|| {
            ...
            assert!(input.read(&mut buffer).unwrap_err().to_string().contains("Not enough data to fill the buffer"));
        })
    }
    ...
    fn reading_with_fill_buffer() {
        sp_io::TestExternalities::default().execute_with(|| {
            ...
            assert!(
                input.read(&mut result_buffer).unwrap_err().to_string().contains("Not enough data to fill the buffer")
            );
        })
    }
    ...
    fn detect_value_deleted_in_state() {
        sp_io::TestExternalities::default().execute_with(|| {
            ...
            assert!(
                Vec::<u8>::decode(&mut input).unwrap_err().to_string().contains("Value doesn't exist in the state?")
            );
