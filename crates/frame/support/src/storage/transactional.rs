...
use mp_runtime::{DispatchError, TransactionOutcome, TransactionalError};
...
where
    ...
    F: FnOnce() -> Result<T, E>,
{
    if is_transactional() { f() } else { with_storage_layer(f) }
}
...
mod tests {
    ...
    fn transaction_limit_should_work() {
        TestExternalities::default().execute_with(|| {
            ...
            assert_noop!(recursive_transactional(256), mp_runtime::TransactionalError::LimitReached);
