...
use crate::metadata_ir::StorageEntryMetadataIR;
use crate::storage::generator::StorageMap as _;
use crate::storage::types::{
    OptionQuery, QueryKindTrait, StorageEntryMetadataBuilder, StorageMap, StorageValue, ValueQuery,
};
use crate::storage::{StorageAppend, StorageDecodeLength, StorageTryAppend};
use crate::traits::{Get, GetDefault, StorageInfo, StorageInfoTrait, StorageInstance};
use crate::Never;
...
impl<P: CountedStorageMapInstance, H, K, V, Q, O, M> MapWrapper for CountedStorageMap<P, H, K, V, Q, O, M> {
...
impl<Prefix: CountedStorageMapInstance> crate::storage::PrefixIteratorOnRemoval for OnRemovalCounterUpdate<Prefix> {
    ...
    MaxValues: Get<Option<u32>>,
{
    ...
    pub fn migrate_key<OldHasher: crate::hash::StorageHasher, KeyArg: EncodeLike<Key>>(key: KeyArg) -> Option<Value> {
...
impl<Prefix, Hasher, Key, Value, QueryKind, OnEmpty, MaxValues> crate::traits::PartialStorageInfoTrait
    for CountedStorageMap<Prefix, Hasher, Key, Value, QueryKind, OnEmpty, MaxValues>
where
    Prefix: CountedStorageMapInstance,
    Hasher: crate::hash::StorageHasher,
    ...
    MaxValues: Get<Option<u32>>,
{
    fn partial_storage_info() -> Vec<StorageInfo> {
        [<Self as MapWrapper>::Map::partial_storage_info(), CounterFor::<Prefix>::storage_info()].concat()
    }
}
...
mod test {
    ...
    fn test_iter_drain_translate() {
        ...
        TestExternalities::default().execute_with(|| {
            ...
            A::translate::<u32, _>(|key, value| if key == 1 { None } else { Some(key as u32 * value) });
