...
use mp_runtime::SaturatedConversion;
use sp_std::prelude::*;

use crate::metadata_ir::{StorageEntryMetadataIR, StorageEntryTypeIR};
use crate::storage::types::{
    EncodeLikeTuple, HasKeyPrefix, HasReversibleKeyPrefix, OptionQuery, QueryKindTrait, StorageEntryMetadataBuilder,
    TupleToEncodedIter,
};
use crate::storage::{KeyGenerator, PrefixIterator, StorageAppend, StorageDecodeLength, StoragePrefixedMap};
use crate::traits::{Get, GetDefault, StorageInfo, StorageInstance};
...
pub struct StorageNMap<Prefix, Key, Value, QueryKind = OptionQuery, OnEmpty = GetDefault, MaxValues = GetDefault>(
    core::marker::PhantomData<(Prefix, Key, Value, QueryKind, OnEmpty, MaxValues)>,
);

impl<Prefix, Key, Value, QueryKind, OnEmpty, MaxValues> crate::storage::generator::StorageNMap<Key, Value>
    ...
    MaxValues: Get<Option<u32>>,
{
    ...
    pub fn clear_prefix<KP>(partial_key: KP, limit: u32, maybe_cursor: Option<&[u8]>) -> sp_io::MultiRemovalResults
    where
        Key: HasKeyPrefix<KP>,
    {
    ...
    MaxValues: Get<Option<u32>>,
{
    ...
    where
        Key: HasReversibleKeyPrefix<KP>,
    {
        <Self as crate::storage::IterableStorageNMap<Key, Value>>::iter_prefix_from(kp, starting_raw_key)
    }
    ...
    /// If you add or remove values whose prefix key is `kp` to the map while doing this, you'll get
    /// undefined results.
    pub fn iter_key_prefix<KP>(kp: KP) -> crate::storage::KeyPrefixIterator<<Key as HasKeyPrefix<KP>>::Suffix>
    where
        Key: HasReversibleKeyPrefix<KP>,
    {
    ...
    where
        Key: HasReversibleKeyPrefix<KP>,
    {
        <Self as crate::storage::IterableStorageNMap<Key, Value>>::iter_key_prefix_from(kp, starting_raw_key)
    }
    ...
    ///
    /// If you add or remove values to the map while doing this, you'll get undefined results.
    pub fn iter_from(starting_raw_key: Vec<u8>) -> crate::storage::PrefixIterator<(Key::Key, Value)> {
    ...
    ///
    /// If you add or remove values to the map while doing this, you'll get undefined results.
    pub fn iter_keys_from(starting_raw_key: Vec<u8>) -> crate::storage::KeyPrefixIterator<Key::Key> {
    ...
    MaxValues: Get<Option<u32>>,
{
    fn storage_info() -> Vec<StorageInfo> {
        vec![StorageInfo {
            ...
            max_size: Some(Key::key_max_encoded_len().saturating_add(Value::max_encoded_len()).saturated_into()),
        }]
    }
}
...
mod test {
    ...
    use super::*;
    use crate::hash::{StorageHasher as _, *};
    use crate::metadata_ir::{StorageEntryModifierIR, StorageHasherIR};
    use crate::storage::types::{Key as NMapKey, ValueQuery};
    ...
    fn test_1_key() {
        ...
        type AValueQueryWithAnOnEmpty = StorageNMap<Prefix, NMapKey<Blake2_128Concat, u16>, u32, ValueQuery, ADefault>;
        ...
        TestExternalities::default().execute_with(|| {
            ...
            B::insert((2,), 10);
            assert_eq!(A::migrate_keys((2,), (Box::new(|key| Blake2_256::hash(key).to_vec()),),), Some(10));
            assert_eq!(A::contains_key((2,)), true);
            assert_eq!(A::get((2,)), Some(10));
    ...
    #[test]
    fn test_2_keys() {
        type A = StorageNMap<Prefix, (NMapKey<Blake2_128Concat, u16>, NMapKey<Twox64Concat, u8>), u32, OptionQuery>;
        type AValueQueryWithAnOnEmpty =
            StorageNMap<Prefix, (NMapKey<Blake2_128Concat, u16>, NMapKey<Twox64Concat, u8>), u32, ValueQuery, ADefault>;
        type B = StorageNMap<Prefix, (NMapKey<Blake2_256, u16>, NMapKey<Twox128, u8>), u32, ValueQuery>;
        type C = StorageNMap<Prefix, (NMapKey<Blake2_128Concat, u16>, NMapKey<Twox64Concat, u8>), u8, ValueQuery>;
        type WithLen = StorageNMap<Prefix, (NMapKey<Blake2_128Concat, u16>, NMapKey<Twox64Concat, u8>), Vec<u32>>;
        ...
        TestExternalities::default().execute_with(|| {
            ...
            assert_eq!(
                A::migrate_keys(
                    (2, 20),
                    (Box::new(|key| Blake2_256::hash(key).to_vec()), Box::new(|key| Twox128::hash(key).to_vec()),),
                ),
                Some(10)
            );
            assert_eq!(A::contains_key((2, 20)), true);
