...
use mp_runtime::traits::{BadOrigin, Get, Member, Morph, TryMorph};
use mp_runtime::Either;
...
/// # use mp_runtime::traits::{parameter_types, Get};
/// # use sp_std::cmp::Ordering;
///
...
impl<OuterOrigin, Argument, EO: EnsureOrigin<OuterOrigin>> EnsureOriginWithArg<OuterOrigin, Argument>
    for AsEnsureOriginWithArg<EO>
{
    /// A return type.
...
impl<OuterOrigin, L: EnsureOrigin<OuterOrigin>, R: EnsureOrigin<OuterOrigin>> EnsureOrigin<OuterOrigin>
    for EitherOfDiverse<L, R>
{
    type Success = Either<L::Success, R::Success>;
    fn try_origin(o: OuterOrigin) -> Result<Self::Success, OuterOrigin> {
        L::try_origin(o).map_or_else(|o| R::try_origin(o).map(Either::Right), |o| Ok(Either::Left(o)))
    }
...
pub trait OriginTrait: Sized {
    ...
    fn as_signed(self) -> Option<Self::AccountId> {
        self.into_caller().into_system().and_then(|s| if let RawOrigin::Signed(who) = s { Some(who) } else { None })
    }
...
mod tests {
    ...
    fn either_of_diverse_works() {
        assert_eq!(
            EitherOfDiverse::<EnsureSuccess<ConstBool<true>>, EnsureSuccess<ConstU8<0>>>::try_origin(())
                .unwrap()
                .left(),
            Some(true)
        );
        assert_eq!(
            EitherOfDiverse::<EnsureSuccess<ConstBool<true>>, EnsureFail<u8>>::try_origin(()).unwrap().left(),
            Some(true)
        );
        assert_eq!(
            EitherOfDiverse::<EnsureFail<bool>, EnsureSuccess<ConstU8<0>>>::try_origin(()).unwrap().right(),
            Some(0u8)
        );
    ...
    fn either_of_works() {
        assert_eq!(
            EitherOf::<EnsureSuccess<ConstBool<true>>, EnsureSuccess<ConstBool<false>>>::try_origin(()).unwrap(),
            true
        );
        assert_eq!(EitherOf::<EnsureSuccess<ConstBool<true>>, EnsureFail<bool>>::try_origin(()).unwrap(), true);
        assert_eq!(EitherOf::<EnsureFail<bool>, EnsureSuccess<ConstBool<false>>>::try_origin(()).unwrap(), false);
