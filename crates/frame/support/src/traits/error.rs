...
/// [`frame_support::MAX_MODULE_ERROR_ENCODED_SIZE`]. If the pallet error type exceeds this
...
pub trait PalletError: Encode + Decode {
    ...
    /// [`frame_support::MAX_MODULE_ERROR_ENCODED_SIZE`], and if it is, a compilation error
    /// will be thrown.
...
impl<T: PalletError, E: PalletError> PalletError for Result<T, E> {
    const MAX_ENCODED_SIZE: usize =
        if T::MAX_ENCODED_SIZE > E::MAX_ENCODED_SIZE { T::MAX_ENCODED_SIZE } else { E::MAX_ENCODED_SIZE }
            .saturating_add(1);
}
