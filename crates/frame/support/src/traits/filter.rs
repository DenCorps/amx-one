...
macro_rules! impl_filter_stack {
    ($target:ty, $base:ty, $call:ty, $module:ident) => {
        #[cfg(feature = "std")]
        mod $module {
            use $crate::traits::filter::{swap, take, Box, Contains, FilterStack, RefCell, Vec};
            ...
            impl Contains<$call> for $target {
                fn contains(call: &$call) -> bool {
                    <$base>::contains(call) && FILTER.with(|filter| filter.borrow().iter().all(|f| f(call)))
                }
            }
        ...
        #[cfg(not(feature = "std"))]
        mod $module {
            use $crate::traits::{swap, take, Box, Contains, FilterStack, RefCell, Vec};
