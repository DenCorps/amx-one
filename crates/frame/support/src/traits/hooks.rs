...
use mp_runtime::traits::AtLeast32BitUnsigned;
#[cfg(feature = "try-runtime")]
use mp_runtime::TryRuntimeError;
use sp_std::prelude::*;
...
impl<BlockNumber: Copy + AtLeast32BitUnsigned> OnIdle<BlockNumber> for Tuple {
    fn on_idle(n: BlockNumber, remaining_weight: Weight) -> Weight {
        let on_idle_functions: &[fn(BlockNumber, Weight) -> Weight] = &[for_tuples!( #( Tuple::on_idle ),* )];
        let mut weight = Weight::zero();
...
#[cfg(feature = "std")]
pub trait GenesisBuild<T, I = ()>: Default + mp_runtime::traits::MaybeSerializeDeserialize {
    /// The build function is called within an externalities allowing storage APIs.
    /// Thus one can write to storage using regular pallet storages.
    fn build(&self);

    /// Build the storage using `build` inside default storage.
    fn build_storage(&self) -> Result<mp_runtime::Storage, String> {
    ...
    fn assimilate_storage(&self, storage: &mut mp_runtime::Storage) -> Result<(), String> {
