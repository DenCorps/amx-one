...
#[doc(hidden)]
pub use mp_runtime::traits::{
    ConstBool, ConstI128, ConstI16, ConstI32, ConstI64, ConstI8, ConstU128, ConstU16, ConstU32, ConstU64, ConstU8, Get,
    GetDefault, TryCollect, TypedGet,
};
use mp_runtime::DispatchError;
use scale_info::build::Fields;
use scale_info::{meta_type, Path, Type, TypeInfo, TypeParameter};
...
pub trait DefensiveTruncateFrom<T> {
    ...
    /// use mp_runtime::traits::ConstU32;
    ///
    /// let unbound = vec![1, 2];
...
impl<A, B> TryDrop for SameOrOther<A, B> {
    fn try_drop(self) -> Result<(), Self> {
        if let SameOrOther::None = self { Ok(()) } else { Err(self) }
    }
}
...
pub trait ExtrinsicCall: mp_runtime::traits::Extrinsic {
    /// Get the call of the extrinsic.
    fn call(&self) -> &Self::Call;
}

#[cfg(feature = "std")]
impl<Call, Extra> ExtrinsicCall for mp_runtime::testing::TestXt<Call, Extra>
where
...
impl<Address, Call, Signature, Extra> ExtrinsicCall
    for mp_runtime::generic::UncheckedExtrinsic<Address, Call, Signature, Extra>
where
    Extra: mp_runtime::traits::SignedExtension,
{
    fn call(&self) -> &Self::Call {
        &self.function
    }
...
impl<T: Decode> Decode for WrapperOpaque<T> {
    fn decode<I: Input>(input: &mut I) -> Result<Self, codec::Error> {
        Ok(Self(T::decode_all_with_depth_limit(mp_api::MAX_EXTRINSIC_DEPTH, &mut &<Vec<u8>>::decode(input)?[..])?))
    }
...
impl<T: TypeInfo + 'static> TypeInfo for WrapperOpaque<T> {
    ...
    fn type_info() -> Type {
        Type::builder()
            ...
            .composite(Fields::unnamed().field(|f| f.compact::<u32>()).field(|f| f.ty::<T>().type_name("T")))
    }
}
...
impl<T: Decode> WrapperKeepOpaque<T> {
    ...
    pub fn try_decode(&self) -> Option<T> {
        T::decode_all_with_depth_limit(mp_api::MAX_EXTRINSIC_DEPTH, &mut &self.data[..]).ok()
    }
...
mod test {
    ...
    #[test]
    #[cfg(debug_assertions)]
    #[should_panic(expected = "Defensive failure has been triggered!: \"DefensiveTruncateFrom truncating\"")]
    ...
    fn test_opaque_wrapper_decode_limit() {
        let limit = mp_api::MAX_EXTRINSIC_DEPTH as usize;
