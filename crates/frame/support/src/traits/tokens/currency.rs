...
use mp_runtime::traits::MaybeSerializeDeserialize;
use mp_runtime::FixedPointOperand;
use sp_std::fmt::Debug;

use super::imbalance::{Imbalance, SignedImbalance};
use super::misc::{Balance, ExistenceRequirement, WithdrawReasons};
...
impl<AccountId> Currency<AccountId> for () {
    ...
    fn ensure_can_withdraw(_: &AccountId, _: Self::Balance, _: WithdrawReasons, _: Self::Balance) -> DispatchResult {
        Ok(())
    }
    fn transfer(_: &AccountId, _: &AccountId, _: Self::Balance, _: ExistenceRequirement) -> DispatchResult {
        Ok(())
    }
    fn slash(_: &AccountId, _: Self::Balance) -> (Self::NegativeImbalance, Self::Balance) {
        ((), 0)
    }
    fn deposit_into_existing(_: &AccountId, _: Self::Balance) -> Result<Self::PositiveImbalance, DispatchError> {
        Ok(())
    }
    fn resolve_into_existing(_: &AccountId, _: Self::NegativeImbalance) -> Result<(), Self::NegativeImbalance> {
        Ok(())
    }
