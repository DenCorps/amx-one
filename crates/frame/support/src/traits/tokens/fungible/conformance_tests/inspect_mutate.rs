...
use mp_runtime::traits::{Bounded, Zero};
use sp_arithmetic::traits::AtLeast8BitUnsigned;

use crate::traits::fungible::{Inspect, Mutate};
use crate::traits::tokens::{DepositConsequence, Fortitude, Precision, Preservation, Provenance, WithdrawConsequence};
    ...
    AccountId: AtLeast8BitUnsigned,
{
    ...
    match dust_trap {
        Some(dust_trap) => {
            ...
            assert_eq!(T::total_balance(&dust_trap), initial_dust_trap_balance + T::minimum_balance() - 1.into());
            assert_eq!(T::balance(&dust_trap), initial_dust_trap_balance + T::minimum_balance() - 1.into());
        }
        None => {
            // Verify: Total issuance and active issuance are reduced by the dust amount
            assert_eq!(T::total_issuance(), initial_total_issuance - T::minimum_balance() + 1.into());
            assert_eq!(T::active_issuance(), initial_active_issuance - T::minimum_balance() + 1.into());
        }
    }
}
