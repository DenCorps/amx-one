...
pub trait Inspect<AccountId>: super::Inspect<AccountId> {
    ...
    fn ensure_can_hold(reason: &Self::Reason, who: &AccountId, amount: Self::Balance) -> DispatchResult {
        ...
        ensure!(amount <= Self::reducible_balance(who, Protect, Force), TokenError::FundsUnavailable);
        Ok(())
    }
...
pub trait Balanced<AccountId>: super::Balanced<AccountId> + Unbalanced<AccountId> {
    ...
    fn slash(
        ...
        let decrease = Self::decrease_balance_on_hold(reason, who, amount, BestEffort).unwrap_or(Default::default());
        let credit = Imbalance::<Self::Balance, Self::OnDropCredit, Self::OnDropDebt>::new(decrease);
