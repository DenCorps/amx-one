...
use mp_runtime::{DispatchError, DispatchResult};
use sp_core::Get;

use super::*;
use crate::traits::tokens::{
    fungibles, DepositConsequence, Fortitude, Imbalance as ImbalanceT, Precision, Preservation, Provenance,
...
/// a single item.
pub struct ItemOf<F: fungibles::Inspect<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>(
    sp_std::marker::PhantomData<(F, A, AccountId)>,
);

impl<F: fungibles::Inspect<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
...
impl<F: fungibles::InspectHold<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
...
impl<F: fungibles::InspectFreeze<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
...
impl<F: fungibles::Unbalanced<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
    Unbalanced<AccountId> for ItemOf<F, A, AccountId>
{
    ...
    fn decrease_balance(
        ...
        <F as fungibles::Unbalanced<AccountId>>::decrease_balance(A::get(), who, amount, precision, preservation, force)
    }
    fn increase_balance(
        who: &AccountId,
        amount: Self::Balance,
...
impl<F: fungibles::UnbalancedHold<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
    UnbalancedHold<AccountId> for ItemOf<F, A, AccountId>
{
    ...
    fn decrease_balance_on_hold(
        ...
        <F as fungibles::UnbalancedHold<AccountId>>::decrease_balance_on_hold(A::get(), reason, who, amount, precision)
    }
    fn increase_balance_on_hold(
        ...
        <F as fungibles::UnbalancedHold<AccountId>>::increase_balance_on_hold(A::get(), reason, who, amount, precision)
    }
}

impl<F: fungibles::Mutate<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
...
impl<F: fungibles::MutateHold<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
    MutateHold<AccountId> for ItemOf<F, A, AccountId>
{
    ...
    fn burn_held(
        ...
        <F as fungibles::MutateHold<AccountId>>::burn_held(A::get(), reason, who, amount, precision, force)
    }
    fn transfer_on_hold(
        reason: &Self::Reason,
        source: &AccountId,
...
impl<F: fungibles::MutateFreeze<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
...
> HandleImbalanceDrop<Balance> for ConvertImbalanceDropHandler<AccountId, Balance, AssetIdType, AssetId, Handler>
{
...
impl<
    F: fungibles::Inspect<AccountId> + fungibles::Unbalanced<AccountId> + fungibles::Balanced<AccountId>,
    A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>,
    AccountId,
> Balanced<AccountId> for ItemOf<F, A, AccountId>
{
    type OnDropDebt = ConvertImbalanceDropHandler<AccountId, Self::Balance, F::AssetId, A, F::OnDropDebt>;
    type OnDropCredit = ConvertImbalanceDropHandler<AccountId, Self::Balance, F::AssetId, A, F::OnDropCredit>;
    fn deposit(
        who: &AccountId,
        value: Self::Balance,
        precision: Precision,
    ...
    fn resolve(who: &AccountId, credit: Credit<AccountId, Self>) -> Result<(), Credit<AccountId, Self>> {
        ...
        <F as fungibles::Balanced<AccountId>>::resolve(who, credit).map_err(|credit| Imbalance::new(credit.peek()))
    }
    fn settle(
        who: &AccountId,
    ...
    fn withdraw(
        ...
        <F as fungibles::Balanced<AccountId>>::withdraw(A::get(), who, value, precision, preservation, force)
            .map(|credit| Imbalance::new(credit.peek()))
    }
}

impl<F: fungibles::BalancedHold<AccountId>, A: Get<<F as fungibles::Inspect<AccountId>>::AssetId>, AccountId>
    BalancedHold<AccountId> for ItemOf<F, A, AccountId>
{
    fn slash(
        ...
        let (credit, amount) = <F as fungibles::BalancedHold<AccountId>>::slash(A::get(), reason, who, amount);
