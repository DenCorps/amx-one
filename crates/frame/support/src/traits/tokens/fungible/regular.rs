...
use crate::traits::tokens::Imbalance as ImbalanceT;
...
pub trait Unbalanced<AccountId>: Inspect<AccountId> {
    ...
    fn increase_balance(
        ...
        if new_balance < Self::minimum_balance() {
            ...
            if let BestEffort = precision { Ok(Default::default()) } else { Err(TokenError::BelowMinimum.into()) }
        } else {
...
impl<AccountId, U: Unbalanced<AccountId>> HandleImbalanceDrop<U::Balance> for IncreaseIssuance<AccountId, U> {
...
impl<AccountId, U: Unbalanced<AccountId>> HandleImbalanceDrop<U::Balance> for DecreaseIssuance<AccountId, U> {
    fn handle(amount: U::Balance) {
