...
use mp_runtime::{DispatchError, DispatchResult, Saturating, TokenError};
...
use super::*;
use crate::ensure;
use crate::traits::tokens::DepositConsequence::Success;
use crate::traits::tokens::Fortitude::{self, Force};
use crate::traits::tokens::Precision::{self, BestEffort, Exact};
use crate::traits::tokens::Preservation::{self, Protect};
use crate::traits::tokens::Provenance::Extant;
use crate::traits::tokens::Restriction::{self, Free, OnHold};
...
pub trait Inspect<AccountId>: super::Inspect<AccountId> {
    ...
    fn ensure_can_hold(
        ...
        ensure!(amount <= Self::reducible_balance(asset, who, Protect, Force), TokenError::FundsUnavailable);
        Ok(())
    }
...
pub trait Balanced<AccountId>: super::Balanced<AccountId> + Unbalanced<AccountId> {
    ...
    fn slash(
        ...
        let decrease = Self::decrease_balance_on_hold(asset.clone(), reason, who, amount, BestEffort)
            .unwrap_or(Default::default());
        let credit = Imbalance::<Self::AssetId, Self::Balance, Self::OnDropCredit, Self::OnDropDebt>::new(
            asset.clone(),
            decrease,
        );
...
pub trait Mutate<AccountId>: Inspect<AccountId> + super::Unbalanced<AccountId> + Unbalanced<AccountId> {
    ...
    fn release(
        ...
        // very wrong.
        ensure!(Self::can_deposit(asset.clone(), who, amount, Extant) == Success, TokenError::CannotCreate);
    ...
    fn burn_held(
        ...
        let amount = Self::decrease_balance_on_hold(asset.clone(), reason, who, amount, precision)?;
        Self::set_total_issuance(asset.clone(), Self::total_issuance(asset.clone()).saturating_sub(amount));
    ...
    fn transfer_on_hold(
        ...
        // We want to make sure we can deposit the amount in advance. If we can't then something is
        // very wrong.
        ensure!(Self::can_deposit(asset.clone(), dest, amount, Extant) == Success, TokenError::CannotCreate);
        ensure!(mode == Free || Self::hold_available(asset.clone(), reason, dest), TokenError::CannotCreateHold);

        let amount = Self::decrease_balance_on_hold(asset.clone(), reason, source, amount, precision)?;
    ...
    fn transfer_and_hold(
        ...
        let actual = Self::decrease_balance(asset.clone(), source, amount, precision, expendability, force)?;
