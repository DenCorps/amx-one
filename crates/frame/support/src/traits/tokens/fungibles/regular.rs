...
use mp_runtime::traits::Saturating;
use mp_runtime::{ArithmeticError, TokenError};
...
use crate::ensure;
use crate::traits::tokens::misc::Fortitude::{self, Force, Polite};
use crate::traits::tokens::misc::Precision::{self, BestEffort, Exact};
use crate::traits::tokens::misc::Preservation::{self, Expendable};
use crate::traits::tokens::misc::Provenance::{self, Extant};
use crate::traits::tokens::misc::{Balance, DepositConsequence, WithdrawConsequence};
use crate::traits::tokens::AssetId;
use crate::traits::{SameOrOther, TryDrop};
...
pub trait Inspect<AccountId>: Sized {
    ...
    /// the consequence.
    fn can_withdraw(asset: Self::AssetId, who: &AccountId, amount: Self::Balance)
...
pub trait Unbalanced<AccountId>: Inspect<AccountId> {
    ...
    fn handle_raw_dust(asset: Self::AssetId, amount: Self::Balance) {
        Self::handle_dust(Dust(asset.clone(), amount.min(Self::minimum_balance(asset).saturating_sub(One::one()))))
    }
    ...
    fn increase_balance(
        ...
        if new_balance < Self::minimum_balance(asset.clone()) {
            ...
            if let BestEffort = precision { Ok(Self::Balance::default()) } else { Err(TokenError::BelowMinimum.into()) }
        } else {
...
pub trait Mutate<AccountId>: Inspect<AccountId> + Unbalanced<AccountId> {
    ...
    fn mint_into(asset: Self::AssetId, who: &AccountId, amount: Self::Balance) -> Result<Self::Balance, DispatchError> {
        Self::total_issuance(asset.clone()).checked_add(&amount).ok_or(ArithmeticError::Overflow)?;
        let actual = Self::increase_balance(asset.clone(), who, amount, Exact)?;
        Self::set_total_issuance(asset.clone(), Self::total_issuance(asset.clone()).saturating_add(actual));
    ...
    fn burn_from(
        ...
        Self::total_issuance(asset.clone()).checked_sub(&actual).ok_or(ArithmeticError::Overflow)?;
        let actual = Self::decrease_balance(asset.clone(), who, actual, BestEffort, Expendable, force)?;
        Self::set_total_issuance(asset.clone(), Self::total_issuance(asset.clone()).saturating_sub(actual));
    ...
    fn shelve(asset: Self::AssetId, who: &AccountId, amount: Self::Balance) -> Result<Self::Balance, DispatchError> {
        ...
        Self::total_issuance(asset.clone()).checked_sub(&actual).ok_or(ArithmeticError::Overflow)?;
        let actual = Self::decrease_balance(asset.clone(), who, actual, BestEffort, Expendable, Polite)?;
        Self::set_total_issuance(asset.clone(), Self::total_issuance(asset.clone()).saturating_sub(actual));
...
pub trait Balanced<AccountId>: Inspect<AccountId> + Unbalanced<AccountId> {
    ...
    fn rescind(asset: Self::AssetId, amount: Self::Balance) -> Debt<AccountId, Self> {
        ...
        Imbalance::<Self::AssetId, Self::Balance, Self::OnDropDebt, Self::OnDropCredit>::new(asset, delta)
    }
    ...
    fn issue(asset: Self::AssetId, amount: Self::Balance) -> Credit<AccountId, Self> {
        ...
        Imbalance::<Self::AssetId, Self::Balance, Self::OnDropCredit, Self::OnDropDebt>::new(asset, delta)
    }
    ...
    fn deposit(
        ...
        Ok(Imbalance::<Self::AssetId, Self::Balance, Self::OnDropDebt, Self::OnDropCredit>::new(asset, increase))
    }
    ...
    fn withdraw(
        ...
        let decrease = Self::decrease_balance(asset.clone(), who, value, precision, preservation, force)?;
        Self::done_withdraw(asset.clone(), who, decrease);
        Ok(Imbalance::<Self::AssetId, Self::Balance, Self::OnDropCredit, Self::OnDropDebt>::new(asset, decrease))
    }
