...
pub trait Inspect<AccountId> {
    ...
    fn typed_attribute<K: Encode, V: Decode>(item: &Self::ItemId, key: &K) -> Option<V> {
        key.using_encoded(|d| Self::attribute(item, d)).and_then(|v| V::decode(&mut &v[..]).ok())
    }
    ...
    fn typed_custom_attribute<K: Encode, V: Decode>(account: &AccountId, item: &Self::ItemId, key: &K) -> Option<V> {
        key.using_encoded(|d| Self::custom_attribute(account, item, d)).and_then(|v| V::decode(&mut &v[..]).ok())
    }
    ...
    fn typed_system_attribute<K: Encode, V: Decode>(item: &Self::ItemId, key: &K) -> Option<V> {
        key.using_encoded(|d| Self::system_attribute(item, d)).and_then(|v| V::decode(&mut &v[..]).ok())
    }
...
impl<F: nonfungibles::Inspect<AccountId>, A: Get<<F as nonfungibles::Inspect<AccountId>>::CollectionId>, AccountId>
    Inspect<AccountId> for ItemOf<F, A, AccountId>
{
    ...
    fn typed_custom_attribute<K: Encode, V: Decode>(account: &AccountId, item: &Self::ItemId, key: &K) -> Option<V> {
        <F as nonfungibles::Inspect<AccountId>>::typed_custom_attribute(account, &A::get(), item, key)
    }
    fn typed_system_attribute<K: Encode, V: Decode>(item: &Self::ItemId, key: &K) -> Option<V> {
...
> InspectEnumerable<AccountId> for ItemOf<F, A, AccountId>
{
    ...
    type OwnedIterator = <F as nonfungibles::InspectEnumerable<AccountId>>::OwnedInCollectionIterator;
...
> Mutate<AccountId, ItemConfig> for ItemOf<F, A, AccountId>
{
    ...
    fn set_attribute(item: &Self::ItemId, key: &[u8], value: &[u8]) -> DispatchResult {
        <F as nonfungibles::Mutate<AccountId, ItemConfig>>::set_attribute(&A::get(), item, key, value)
    }
    fn set_typed_attribute<K: Encode, V: Encode>(item: &Self::ItemId, key: &K, value: &V) -> DispatchResult {
        <F as nonfungibles::Mutate<AccountId, ItemConfig>>::set_typed_attribute(&A::get(), item, key, value)
    }
    ...
    fn clear_typed_attribute<K: Encode>(item: &Self::ItemId, key: &K) -> DispatchResult {
        <F as nonfungibles::Mutate<AccountId, ItemConfig>>::clear_typed_attribute(&A::get(), item, key)
    }
}

impl<F: nonfungibles::Transfer<AccountId>, A: Get<<F as nonfungibles::Inspect<AccountId>>::CollectionId>, AccountId>
