...
pub trait Inspect<AccountId> {
    ...
    fn typed_attribute<K: Encode, V: Decode>(
        ...
        key.using_encoded(|d| Self::attribute(collection, item, d)).and_then(|v| V::decode(&mut &v[..]).ok())
    }
    ...
    fn typed_collection_attribute<K: Encode, V: Decode>(collection: &Self::CollectionId, key: &K) -> Option<V> {
        key.using_encoded(|d| Self::collection_attribute(collection, d)).and_then(|v| V::decode(&mut &v[..]).ok())
    }
...
pub trait Mutate<AccountId>: Inspect<AccountId> {
    ...
    fn mint_into(_collection: &Self::CollectionId, _item: &Self::ItemId, _who: &AccountId) -> DispatchResult {
