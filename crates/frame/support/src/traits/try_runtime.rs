    ...
    fn fmt(&self, f: &mut sp_std::fmt::Formatter<'_>) -> sp_std::fmt::Result {
        match self {
            ...
            Select::Only(x) => write!(
                f,
                "Only({:?})",
                x.iter().map(|x| sp_std::str::from_utf8(x).unwrap_or("<invalid?>")).collect::<Vec<_>>(),
            ),
...
impl<BlockNumber: Clone + sp_std::fmt::Debug + AtLeast32BitUnsigned> TryState<BlockNumber> for Tuple {
    ...
    fn try_state(n: BlockNumber, targets: Select) -> Result<(), TryRuntimeError> {
        match targets {
            ...
            Select::RoundRobin(len) => {
                ...
                let skip: u32 = skip.try_into().unwrap_or_else(|_| mp_runtime::traits::Bounded::max_value());
                let mut result = Ok(());
            ...
            Select::Only(ref pallet_names) => {
                let try_state_fns: &[(&'static str, fn(BlockNumber, Select) -> Result<(), TryRuntimeError>)] =
                    &[for_tuples!(
