...
use mp_runtime::generic::Era;
use mp_runtime::traits::{DispatchInfoOf, SaturatedConversion, SignedExtension};
...
impl<T: Config + Send + Sync> SignedExtension for CheckMortality<T> {
    ...
    fn validate(
        ...
        Ok(ValidTransaction { longevity: valid_till.saturating_sub(current_u64), ..Default::default() })
    }
...
mod tests {
    ...
    fn signed_ext_check_era_should_change_longevity() {
        new_test_ext().execute_with(|| {
            ...
            let len = 0_usize;
            let ext = (crate::CheckWeight::<Test>::new(), CheckMortality::<Test>::from(Era::mortal(16, 256)));
