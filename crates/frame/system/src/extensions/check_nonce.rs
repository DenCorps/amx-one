...
use mp_runtime::traits::{DispatchInfoOf, Dispatchable, One, SignedExtension};
    ...
    fn signed_ext_check_nonce_works() {
        new_test_ext().execute_with(|| {
            crate::Account::<Test>::insert(
                1,
                crate::AccountInfo { nonce: 1, consumers: 0, providers: 0, sufficients: 0, data: 0 },
            );
            let info = DispatchInfo::default();
            let len = 0_usize;
            // stale
            assert_noop!(CheckNonce::<Test>(0).validate(&1, CALL, &info, len), InvalidTransaction::Stale);
            assert_noop!(CheckNonce::<Test>(0).pre_dispatch(&1, CALL, &info, len), InvalidTransaction::Stale);
            // correct
            ...
            assert_noop!(CheckNonce::<Test>(5).pre_dispatch(&1, CALL, &info, len), InvalidTransaction::Future);
        })
    }
}
