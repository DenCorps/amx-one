...
use mp_runtime::traits::{DispatchInfoOf, Dispatchable, PostDispatchInfoOf, SignedExtension};
use mp_runtime::transaction_validity::{InvalidTransaction, TransactionValidity, TransactionValidityError};
use mp_runtime::DispatchResult;
    ...
    T::RuntimeCall: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    fn check_extrinsic_weight(info: &DispatchInfoOf<T::RuntimeCall>) -> Result<(), TransactionValidityError> {
        let max = T::BlockWeights::get().get(info.class).max_extrinsic;
        match max {
            Some(max) if info.weight.any_gt(max) => Err(InvalidTransaction::ExhaustsResources.into()),
            _ => Ok(()),
        }
    }
    ...
    fn check_block_length(info: &DispatchInfoOf<T::RuntimeCall>, len: usize) -> Result<u32, TransactionValidityError> {
    ...
    pub fn do_pre_dispatch(info: &DispatchInfoOf<T::RuntimeCall>, len: usize) -> Result<(), TransactionValidityError> {
    ...
    Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    if limit_per_class.max_total.is_none() && limit_per_class.reserved.is_none() {
        ...
        all_weight.checked_accrue(extrinsic_weight, info.class).map_err(|_| InvalidTransaction::ExhaustsResources)?;
    }
    ...
    if all_weight.total().any_gt(maximum_weight.max_block) {
        match limit_per_class.reserved {
            ...
            Some(reserved) if per_class.any_gt(reserved) => return Err(InvalidTransaction::ExhaustsResources.into()),
...
mod tests {
    ...
    use crate::{AllExtrinsicsLen, BlockWeight, DispatchClass};
    ...
    fn normal_weight_limit() -> Weight {
        block_weights().get(DispatchClass::Normal).max_total.unwrap_or_else(|| block_weights().max_block)
    }
    ...
    fn mandatory_extrinsic_doesnt_care_about_limits() {
        fn check(call: impl FnOnce(&DispatchInfo, usize)) {
            new_test_ext().execute_with(|| {
                let max = DispatchInfo { weight: Weight::MAX, class: DispatchClass::Mandatory, ..Default::default() };
                let len = 0_usize;
    ...
    fn normal_extrinsic_limited_by_maximum_extrinsic_weight() {
        new_test_ext().execute_with(|| {
            let max = DispatchInfo {
                weight: block_weights().get(DispatchClass::Normal).max_extrinsic.unwrap() + Weight::from_parts(1, 0),
    ...
    fn operational_extrinsic_limited_by_operational_space_limit() {
        new_test_ext().execute_with(|| {
            ...
            let operational_limit =
                weights.get(DispatchClass::Operational).max_total.unwrap_or_else(|| weights.max_block);
            let base_weight = weights.get(DispatchClass::Operational).base_extrinsic;

            let weight = operational_limit - base_weight;
            let okay = DispatchInfo { weight, class: DispatchClass::Operational, ..Default::default() };
            let max = DispatchInfo {
    ...
    fn full_block_with_normal_and_operational() {
        new_test_ext().execute_with(|| {
            ...
            let max_normal = DispatchInfo { weight: Weight::from_parts(753, 0), ..Default::default() };
    ...
    fn operational_works_on_full_block() {
        new_test_ext().execute_with(|| {
            ...
            let dispatch_normal =
                DispatchInfo { weight: Weight::from_parts(251, 0), class: DispatchClass::Normal, ..Default::default() };
    ...
    fn signed_ext_check_weight_works_operational_tx() {
        new_test_ext().execute_with(|| {
            ...
            BlockWeight::<Test>::mutate(|current_weight| current_weight.set(normal_limit, DispatchClass::Normal));
            // will not fit.
            assert_err!(
    ...
    fn signed_ext_check_weight_block_size_works() {
        new_test_ext().execute_with(|| {
            ...
            let op = DispatchInfo { weight: Weight::zero(), class: DispatchClass::Operational, pays_fee: Pays::Yes };
    ...
    fn signed_ext_check_weight_works_normal_tx() {
        new_test_ext().execute_with(|| {
            ...
            let medium = DispatchInfo { weight: normal_limit - base_extrinsic, ..Default::default() };
            let big =
                DispatchInfo { weight: normal_limit - base_extrinsic + Weight::from_parts(1, 0), ..Default::default() };
            let len = 0_usize;

            let reset_check_weight = |i, f, s| {
                BlockWeight::<Test>::mutate(|current_weight| current_weight.set(s, DispatchClass::Normal));
    ...
    fn signed_ext_check_weight_refund_works() {
        new_test_ext().execute_with(|| {
            ...
            let post_info =
                PostDispatchInfo { actual_weight: Some(Weight::from_parts(128, 0)), pays_fee: Default::default() };
            let len = 0_usize;
            let base_extrinsic = block_weights().get(DispatchClass::Normal).base_extrinsic;
            ...
            BlockWeight::<Test>::mutate(|current_weight| {
                ...
                current_weight.set(Weight::from_parts(256, 0) - base_extrinsic, DispatchClass::Normal);
            });

            let pre = CheckWeight::<Test>(PhantomData).pre_dispatch(&1, CALL, &info, len).unwrap();
            assert_eq!(BlockWeight::<Test>::get().total(), info.weight + Weight::from_parts(256, 0));
    ...
    fn signed_ext_check_weight_actual_weight_higher_than_max_is_capped() {
        new_test_ext().execute_with(|| {
            let info = DispatchInfo { weight: Weight::from_parts(512, 0), ..Default::default() };
            let post_info =
                PostDispatchInfo { actual_weight: Some(Weight::from_parts(700, 0)), pays_fee: Default::default() };
            let len = 0_usize;
            ...
            assert_eq!(
                BlockWeight::<Test>::get().total(),
                info.weight + Weight::from_parts(128, 0) + block_weights().get(DispatchClass::Normal).base_extrinsic,
            );
    ...
    fn no_max_total_should_still_be_limited_by_max_block() {
        ...
        // fits into reserved
        let mandatory1 =
            DispatchInfo { weight: Weight::from_parts(5, 0), class: DispatchClass::Mandatory, ..Default::default() };
        // does not fit into reserved and the block is full.
        let mandatory2 =
            DispatchInfo { weight: Weight::from_parts(6, 0), class: DispatchClass::Mandatory, ..Default::default() };
        ...
        assert_err!(
            calculate_consumed_weight::<<Test as Config>::RuntimeCall>(maximum_weight, all_weight, &mandatory2),
            InvalidTransaction::ExhaustsResources
        );
    }
}
