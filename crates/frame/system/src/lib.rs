...
use frame_support::dispatch::{
    extract_actual_pays_fee, extract_actual_weight, DispatchClass, DispatchInfo, DispatchResult,
    DispatchResultWithPostInfo, PerDispatchClass,
};
use frame_support::storage::{self, StorageStreamIter};
#[cfg(feature = "std")]
use frame_support::traits::GenesisBuild;
use frame_support::traits::{
    ConstU32, Contains, EnsureOrigin, Get, HandleLifetime, OnKilledAccount, OnNewAccount, OriginTrait, PalletInfo,
    SortedMembers, StoredMap, TypedGet,
};
use frame_support::Parameter;
#[cfg(feature = "runtime-benchmarks")]
use mp_runtime::traits::TrailingZeroInput;
use mp_runtime::traits::{
    self, AtLeast32Bit, AtLeast32BitUnsigned, BadOrigin, BlockNumberProvider, Bounded, CheckEqual, Dispatchable, Hash,
    Lookup, LookupError, MaybeDisplay, MaybeSerializeDeserialize, Member, One, Saturating, SimpleBitOps, StaticLookup,
    Zero,
};
use mp_runtime::{generic, DispatchError, RuntimeDebug};
...
pub use extensions::check_genesis::CheckGenesis;
pub use extensions::check_mortality::CheckMortality;
// Backward compatible re-export.
pub use extensions::check_mortality::CheckMortality as CheckEra;
pub use extensions::check_non_zero_sender::CheckNonZeroSender;
pub use extensions::check_nonce::CheckNonce;
pub use extensions::check_spec_version::CheckSpecVersion;
pub use extensions::check_tx_version::CheckTxVersion;
pub use extensions::check_weight::CheckWeight;
...
pub mod pallet {
    ...
    pub type Account<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, AccountInfo<T::Index, T::AccountData>, ValueQuery>;
    ...
    pub type BlockHash<T: Config> = StorageMap<_, Twox64Concat, T::BlockNumber, T::Hash, ValueQuery>;
...
impl GenesisConfig {
    ...
    pub fn build_storage<T: Config>(&self) -> Result<mp_runtime::Storage, String> {
    ...
    ///
    /// Kept in order not to break dependency.
    pub fn assimilate_storage<T: Config>(&self, storage: &mut mp_runtime::Storage) -> Result<(), String> {
...
/// happened.
#[derive(mp_runtime::RuntimeDebug, Encode, Decode, TypeInfo)]
#[cfg_attr(feature = "std", derive(PartialEq))]
pub struct LastRuntimeUpgradeInfo {
    pub spec_version: codec::Compact<u32>,
    pub spec_name: mp_runtime::RuntimeString,
}
...
impl LastRuntimeUpgradeInfo {
    ...
    pub fn was_upgraded(&self, current: &mp_version::RuntimeVersion) -> bool {
        current.spec_version > self.spec_version.0 || current.spec_name != self.spec_name
    }
}

impl From<mp_version::RuntimeVersion> for LastRuntimeUpgradeInfo {
    fn from(version: mp_version::RuntimeVersion) -> Self {
...
pub struct EnsureRootWithSuccess<AccountId, Success>(sp_std::marker::PhantomData<(AccountId, Success)>);
impl<O: Into<Result<RawOrigin<AccountId>, O>> + From<RawOrigin<AccountId>>, AccountId, Success: TypedGet>
...
pub struct EnsureWithSuccess<Ensure, AccountId, Success>(sp_std::marker::PhantomData<(Ensure, AccountId, Success)>);
    ...
    for EnsureSigned<AccountId>
{
    ...
    #[cfg(feature = "runtime-benchmarks")]
    fn try_successful_origin() -> Result<O, ()> {
        let zero_account_id = AccountId::decode(&mut TrailingZeroInput::zeroes()).map_err(|_| ())?;
...
impl<T: Config> Pallet<T> {
        ...
        Account::<T>::try_mutate_exists(who, |maybe_account| {
            if let Some(mut account) = maybe_account.take() {
                if account.providers == 0 {
                    ...
                    log::error!(target: LOG_TARGET, "Logic error: Unexpected underflow in reducing provider",);
                    account.providers = 1;
                }
                ...
                log::error!(target: LOG_TARGET, "Logic error: Account already dead when reducing provider",);
                Ok(DecRefStatus::Reaped)
            }
        })
    }
        ...
        Account::<T>::mutate_exists(who, |maybe_account| {
            if let Some(mut account) = maybe_account.take() {
                if account.sufficients == 0 {
                    // Logic error - cannot decrement beyond zero.
                    log::error!(target: LOG_TARGET, "Logic error: Unexpected underflow in reducing sufficients",);
                }
    ...
    pub fn dec_consumers(who: &T::AccountId) {
        Account::<T>::mutate(who, |a| {
            if a.consumers > 0 {
                ...
                log::error!(target: LOG_TARGET, "Logic error: Unexpected underflow in reducing consumer",);
            }
        })
    }
    ...
    pub fn finalize() -> T::Header {
        log::debug!(
            target: LOG_TARGET,
            "[{:?}] {} extrinsics, length: {} (normal {}%, op: {}%, mandatory {}%) / normal weight:{} ({}%) op weight \
             {} ({}%) / mandatory weight {} ({}%)",
            Self::block_number(),
            Self::extrinsic_index().unwrap_or_default(),
            Self::all_extrinsics_len(),
            mp_runtime::Percent::from_rational(
                Self::all_extrinsics_len(),
        ...
        let extrinsics = (0..ExtrinsicCount::<T>::take().unwrap_or_default()).map(ExtrinsicData::<T>::take).collect();
    ...
    pub fn events() -> Vec<EventRecord<T::RuntimeEvent, T::Hash>> {
        debug_assert!(!Self::block_number().is_zero(), "events not registered at the genesis block");
    ...
    pub fn set_block_consumed_resources(weight: Weight, len: usize) {
        BlockWeight::<T>::mutate(|current_weight| current_weight.set(weight, DispatchClass::Normal));
    ...
    pub fn assert_last_event(event: T::RuntimeEvent) {
        ...
        assert_eq!(last_event, event, "expected event {event:?} is not equal to the last event {last_event:?}",);
    }
    ...
    pub fn note_applied_extrinsic(r: &DispatchResultWithPostInfo, mut info: DispatchInfo) {
        info.weight =
            extract_actual_weight(r, &info).saturating_add(T::BlockWeights::get().get(info.class).base_extrinsic);
        info.pays_fee = extract_actual_pays_fee(r, &info);

        Self::deposit_event(match r {
            Ok(_) => Event::ExtrinsicSuccess { dispatch_info: info },
            Err(err) => {
                log::trace!(target: LOG_TARGET, "Extrinsic failed at block({:?}): {:?}", Self::block_number(), err,);
    ...
    pub fn can_set_code(code: &[u8]) -> Result<(), mp_runtime::DispatchError> {
