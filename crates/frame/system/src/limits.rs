...
use mp_runtime::traits::Bounded;
use mp_runtime::{Perbill, RuntimeDebug};
...
impl BlockLength {
    ...
    pub fn max_with_normal_ratio(max: u32, normal: Perbill) -> Self {
        Self { max: PerDispatchClass::new(|class| if class == DispatchClass::Normal { normal * max } else { max }) }
    }
}
...
impl BlockWeights {
    ...
    pub fn validate(self) -> ValidationResult {
        ...
        if error.has_errors { Err(error) } else { Ok(self) }
    }
        ...
        BlockWeightsBuilder {
            weights: BlockWeights {
                ...
                per_class: PerDispatchClass::new(|class| {
                    let initial = if class == DispatchClass::Mandatory { None } else { Some(Weight::zero()) };
                    WeightsPerClass {
...
impl BlockWeightsBuilder {
    ...
    pub fn for_class(mut self, class: impl OneOrMany<DispatchClass>, action: impl Fn(&mut WeightsPerClass)) -> Self {
    ...
    pub fn build_or_panic(self) -> BlockWeights {
        self.build()
            .expect("Builder finished with `build_or_panic`; The panic is expected if runtime weights are not correct")
    }
}
