...
use mp_runtime::app_crypto::RuntimeAppPublic;
use mp_runtime::traits::{Extrinsic as ExtrinsicT, IdentifyAccount, One};
...
>
{
    ...
    fn send_single_signed_transaction(&self, account: &Account<T>, call: LocalCall) -> Option<Result<(), ()>> {
        ...
        let (call, signature) =
            T::create_transaction::<C>(call.into(), account.public.clone(), account.id.clone(), account_data.nonce)?;
...
mod tests {
    use codec::Decode;
    use mp_runtime::testing::{TestSignature, TestXt, UintAuthorityId};
