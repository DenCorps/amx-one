...
use frame_support::dispatch::{Pays, PostDispatchInfo, WithPostDispatchInfo};
use frame_support::traits::WhitelistedStorageKeys;
use frame_support::{assert_noop, assert_ok};
use mock::{RuntimeOrigin, *};
use mp_runtime::traits::{BlakeTwo256, Header};
use mp_runtime::{DispatchError, DispatchErrorWithPostInfo};
...
fn check_whitelist() {
    let whitelist: BTreeSet<String> =
        AllPalletsWithSystem::whitelisted_storage_keys().iter().map(|s| HexDisplay::from(&s.key).to_string()).collect();
...
fn deposit_event_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            System::events(),
            vec![EventRecord { phase: Phase::Finalization, event: SysEvent::CodeUpdated.into(), topics: vec![] }]
        );

        let normal_base = <Test as crate::Config>::BlockWeights::get().get(DispatchClass::Normal).base_extrinsic;
...
fn deposit_event_uses_actual_weight_and_pays_fee() {
    new_test_ext().execute_with(|| {
        ...
        System::note_applied_extrinsic(&Ok(from_post_weight_info(Some(2_500_000), Pays::Yes)), pre_info);
        System::note_applied_extrinsic(&Ok(Pays::No.into()), pre_info);
        System::note_applied_extrinsic(&Ok(from_post_weight_info(Some(2_500_000), Pays::No)), pre_info);
        ...
        System::note_applied_extrinsic(
            &Err(DispatchErrorWithPostInfo {
                post_info: PostDispatchInfo { actual_weight: Some(Weight::from_parts(800, 0)), pays_fee: Pays::Yes },
                error: DispatchError::BadOrigin,
            }),
            pre_info,
        );
        System::note_applied_extrinsic(
            &Err(DispatchErrorWithPostInfo {
                post_info: PostDispatchInfo { actual_weight: Some(Weight::from_parts(800, 0)), pays_fee: Pays::No },
                error: DispatchError::BadOrigin,
            }),
            pre_info,
        );
        ...
        let operational_base =
            <Test as crate::Config>::BlockWeights::get().get(DispatchClass::Operational).base_extrinsic;
...
fn set_code_checks_works() {
    ...
    let test_data = vec![
        ...
        ("test", 2, 1, Ok(Some(<mock::Test as pallet::Config>::BlockWeights::get().max_block).into())),
        ("test", 0, 1, Err(Error::<Test>::SpecVersionNeedsToIncrease)),
        ("test", 1, 0, Err(Error::<Test>::SpecVersionNeedsToIncrease)),
    ];

    for (spec_name, spec_version, impl_version, expected) in test_data.into_iter() {
        let version = RuntimeVersion { spec_name: spec_name.into(), spec_version, impl_version, ..Default::default() };
...
fn set_code_with_real_wasm_blob() {
    ...
    ext.execute_with(|| {
        System::set_block_number(1);
        System::set_code(RawOrigin::Root.into(), substrate_test_runtime_client::runtime::wasm_binary_unwrap().to_vec())
            .unwrap();

        assert_eq!(
            System::events(),
            vec![EventRecord { phase: Phase::Initialization, event: SysEvent::CodeUpdated.into(), topics: vec![] }],
        );
    });
}
...
fn ensure_signed_stuff_works() {
    ...
    #[cfg(feature = "runtime-benchmarks")]
    {
        let successful_origin: RuntimeOrigin =
            EnsureSigned::try_successful_origin().expect("EnsureSigned has no successful origin required for the test");
        assert_ok!(EnsureSigned::try_origin(successful_origin));

        let successful_origin: RuntimeOrigin = EnsureSignedBy::<Members, _>::try_successful_origin()
