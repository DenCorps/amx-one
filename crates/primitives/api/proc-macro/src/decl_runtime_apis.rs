...
use syn::{
    parse_macro_input, parse_quote, Attribute, FnArg, GenericParam, Generics, Ident, ItemTrait, LitInt, LitStr,
    TraitBound, TraitItem, TraitItemFn,
};

use crate::common::{
    API_VERSION_ATTRIBUTE, BLOCK_GENERIC_IDENT, CHANGED_IN_ATTRIBUTE, CORE_TRAIT_ATTRIBUTE, RENAMED_ATTRIBUTE,
...
use crate::utils::{
    ...
    replace_wild_card_parameter_names, return_type_extract_type, versioned_trait_name, AllowSelfRefInParameters,
};
...
impl Fold for ReplaceBlockWithNodeBlock {
    fn fold_ident(&mut self, input: Ident) -> Ident {
        if input == BLOCK_GENERIC_IDENT { Ident::new("NodeBlock", Span::call_site()) } else { input }
    }
}
...
/// }
/// ```
fn generate_versioned_api_traits(api: ItemTrait, methods: BTreeMap<u64, Vec<TraitItemFn>>) -> Vec<ItemTrait> {
...
fn parse_renamed_attribute(renamed: &Attribute) -> Result<(String, u32)> {
    let err = || {
        Error::new(
            renamed.span(),
            &format!(
                "Unexpected `{RENAMED_ATTRIBUTE}` attribute. The supported format is \
                 `{RENAMED_ATTRIBUTE}(\"old_name\", version_it_was_renamed)`",
            ),
        )
    };
...
fn generate_runtime_decls(decls: &[ItemTrait]) -> Result<TokenStream> {
    ...
    for decl in decls {
                    ...
                    method_version = match parse_runtime_api_version(version_attribute) {
                        Ok(method_api_ver) if method_api_ver < trait_api_version => {
                            ...
                            let mut err1 = Error::new(
                                ...
                                format!(
                                    "Method version `{}` is older than (or equal to) trait version `{}`.Methods can't \
                                     define versions older than the trait version.",
                                    method_ver, trait_ver,
                                ),
                            );
                            ...
                            let err2 = match found_attributes.get(&API_VERSION_ATTRIBUTE) {
                                ...
                                None => {
                                    Error::new(decl_span, "Trait version is not set so it is implicitly equal to 1.")
                                }
                            };
                            err1.combine(err2);
        ...
        let versioned_ident =
            &versioned_api_traits.first().expect("There should always be at least one version.").ident;
...
impl<'a> ToClientSideDecl<'a> {
    fn fold_item_trait_items(&mut self, items: Vec<TraitItem>, trait_generics_num: usize) -> Vec<TraitItem> {
        let mut result = Vec::new();
    ...
    fn fold_trait_item_fn(&mut self, method: TraitItemFn, trait_generics_num: usize) -> (TraitItemFn, TraitItemFn) {
        let crate_ = self.crate_;
    ...
    fn create_method_decl_with_context(&mut self, method: TraitItemFn, trait_generics_num: usize) -> TraitItemFn {
        let crate_ = self.crate_;
    ...
    fn create_method_decl(
        ...
        let (versions, old_names) =
            renames.into_iter().fold((Vec::new(), Vec::new()), |(mut versions, mut old_names), (version, old_name)| {
        ...
        match get_changed_in(&found_attributes) {
            Ok(Some(version)) => {
                ...
                if get_api_version(self.found_attributes).ok() < Some(version) {
                    self.errors.push(
                        Error::new(method.span(), "`changed_in` version can not be greater than the `api_version`")
                            .to_compile_error(),
                    );
                }

                let ident =
                    Ident::new(&format!("{}_before_version_{}", method.sig.ident, version), method.sig.ident.span());
...
fn get_changed_in(found_attributes: &HashMap<&'static str, Attribute>) -> Result<Option<u64>> {
    found_attributes.get(&CHANGED_IN_ATTRIBUTE).map(|v| parse_runtime_api_version(v).map(Some)).unwrap_or(Ok(None))
}
...
impl CheckTraitDecl {
    ...
    fn check_method_declarations<'a>(&mut self, methods: impl Iterator<Item = &'a TraitItemFn>) {
        ...
        methods.into_iter().for_each(|method| {
            ...
            method_to_signature_changed.entry(method.sig.ident.clone()).or_default().push(changed_in);
            ...
            if changed.iter().filter(|c| c.is_none()).count() == 0 {
                self.errors.push(Error::new(
                    ...
                    "There is no 'default' method with this name (without `changed_in` attribute).\nThe 'default' \
                     method is used to call into the latest implementation.",
                ));
            }
        });
    }
    ...
    fn visit_generic_param(&mut self, input: &'ast GenericParam) {
        match input {
            GenericParam::Type(ty) if ty.ident == BLOCK_GENERIC_IDENT => self.errors.push(Error::new(
                input.span(),
                "`Block: BlockT` generic parameter will be added automatically by the `decl_runtime_apis!` macro!",
            )),
            _ => {}
        }
        ...
        if let Some(last_ident) = input.path.segments.last().map(|v| &v.ident) {
            if last_ident == "BlockT" || last_ident == BLOCK_GENERIC_IDENT {
                self.errors.push(Error::new(
                    input.span(),
                    "`Block: BlockT` generic parameter will be added automatically by the `decl_runtime_apis!` macro! \
                     If you try to use a different trait than the substrate `Block` trait, please rename it locally.",
                ))
            }
        }
...
fn decl_runtime_apis_impl_inner(api_decls: &[ItemTrait]) -> Result<TokenStream> {
    ...
    let decl = expander::Expander::new("decl_runtime_apis")
        .dry(std::env::var("mp_api_EXPAND").is_err())
        .verbose(true)
