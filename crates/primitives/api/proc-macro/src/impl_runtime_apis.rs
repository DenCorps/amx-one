...
use syn::{parse_macro_input, parse_quote, Attribute, Ident, ImplItem, ItemImpl, Path, Signature, Type, TypePath};
...
use crate::utils::{
    ...
    extract_parameter_names_types_and_borrows, generate_crate_access, generate_runtime_mod_name_for_trait,
    parse_runtime_api_version, prefix_function_with_trait, versioned_trait_name, AllowSelfRefInParameters,
...
fn generate_dispatch_function(impls: &[ItemImpl]) -> Result<TokenStream> {
    let data = Ident::new("_mp_api_input_data_", Span::call_site());
    let c = generate_crate_access();
    let impl_calls = generate_impl_calls(impls, &data)?.into_iter().map(|(trait_, fn_name, impl_, attrs)| {
...
fn generate_wasm_interface(impls: &[ItemImpl]) -> Result<TokenStream> {
    ...
    let impl_calls = generate_impl_calls(impls, &input)?.into_iter().map(|(trait_, fn_name, impl_, attrs)| {
...
fn extend_with_runtime_decl_path(mut trait_: Path) -> Path {
    let runtime = {
        let trait_name =
            &trait_.segments.last().as_ref().expect("Trait path should always contain at least one item; qed").ident;
...
fn extend_with_api_version(mut trait_: Path, version: Option<u64>) -> Path {
    ...
    let trait_name =
        &mut trait_.segments.last_mut().expect("Trait path should always contain at least one item; qed").ident;
...
impl<'a> Fold for ApiRuntimeImplToApiRuntimeApiImpl<'a> {
    fn fold_type_path(&mut self, input: TypePath) -> TypePath {
        let new_ty_path = if input == *self.runtime_block { parse_quote!(__SrApiBlock__) } else { input };
    ...
    fn fold_item_impl(&mut self, mut input: ItemImpl) -> ItemImpl {
        ...
        input.self_ty = Box::new(parse_quote!( RuntimeApiImpl<__SrApiBlock__, RuntimeApiImplCall> ));

        input.generics.params.push(parse_quote!(
            __SrApiBlock__: #crate_::BlockT + std::panic::UnwindSafe +
                std::panic::RefUnwindSafe
        ));
        input.generics.params.push(parse_quote!( RuntimeApiImplCall: #crate_::CallApiAt<__SrApiBlock__> + 'static ));
...
fn generate_api_impl_for_runtime_api(impls: &[ItemImpl]) -> Result<TokenStream> {
    ...
    for impl_ in impls {
        ...
        let processed_impl = ApiRuntimeImplToApiRuntimeApiImpl { runtime_block }.process(impl_.clone());
...
fn generate_runtime_api_versions(impls: &[ItemImpl]) -> Result<TokenStream> {
    ...
    for impl_ in impls {
        ...
        // Remove the trait
        let trait_ =
            path.segments.pop().expect("extract_impl_trait already checks that this is valid; qed").into_value().ident;

        let span = trait_.span();
        if !processed_traits.insert(trait_) {
            return Err(Error::new(
                span,
                "Two traits with the same name detected! The trait name is used to generate its ID. Please rename one \
                 trait at the declaration!",
            ));
        }
...
fn extract_api_version(attrs: &Vec<Attribute>, span: Span) -> Result<Option<u64>> {
    ...
    let api_ver = attrs.iter().filter(|a| a.path().is_ident(API_VERSION_ATTRIBUTE)).collect::<Vec<_>>();

    if api_ver.len() > 1 {
        return Err(Error::new(
            span,
            format!(
                "Found multiple #[{}] attributes for an API implementation. Each runtime API can have only one \
                 version.",
