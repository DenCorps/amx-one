...
use syn::{parse_macro_input, parse_quote, Attribute, ItemImpl, Pat, Type, TypePath};

use crate::utils::{
    extract_block_type_from_trait_path, extract_impl_trait, extract_parameter_names_types_and_borrows,
    generate_crate_access, return_type_extract_type, AllowSelfRefInParameters, RequireQualifiedTraitPath,
};
    ...
    if is_advanced {
        if param_names.is_empty() {
            return Err(Error::new(
                function_span,
                format!(
                    "If using the `{}` attribute, it is required that the function takes at least one argument, the \
                     `Hash`.",
                    ADVANCED_ATTRIBUTE,
                ),
            ));
        ...
        global_block_type = match global_block_type.take() {
            Some(global_block_type) => {
                if global_block_type == *block_type {
                    ...
                    let mut error =
                        Error::new(block_type.span(), "Block type should be the same between all runtime apis.");
...
fn mock_impl_runtime_apis_impl_inner(api_impls: &[ItemImpl]) -> Result<TokenStream> {
    let GeneratedRuntimeApiImpls { impls, block_type, self_ty } = generate_runtime_api_impls(api_impls)?;
