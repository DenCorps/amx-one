...
use crate::common::CHANGED_IN_ATTRIBUTE;
use crate::utils::{
    extract_impl_trait, filter_cfg_attributes, generate_crate_access, generate_runtime_mod_name_for_trait,
...
pub fn generate_decl_runtime_metadata(decl: &ItemTrait) -> TokenStream2 {
    ...
    for item in &decl.items {
        ...
        let is_changed_in = method.attrs.iter().any(|attr| attr.path().is_ident(CHANGED_IN_ATTRIBUTE));
        if is_changed_in {
            continue;
        }
...
pub fn generate_impl_runtime_metadata(impls: &[ItemImpl]) -> Result<TokenStream2> {
    ...
    let runtime_name = &impls.get(0).expect("Traits should contain at least one implementation; qed").self_ty;
