...
use syn::token::And;
use syn::{
    parse_quote, Attribute, Error, FnArg, GenericArgument, Ident, ImplItem, ItemImpl, Pat, Path, PathArguments, Result,
...
pub fn generate_crate_access() -> TokenStream {
    match crate_name("mp-api") {
        Ok(FoundCrate::Itself) => quote!(mp_api),
...
pub fn generate_runtime_mod_name_for_trait(trait_: &Ident) -> Ident {
    Ident::new(&format!("runtime_decl_for_{}", trait_.to_string().to_snake_case()), Span::call_site())
}
...
pub fn replace_wild_card_parameter_names(input: &mut Signature) {
    ...
    input.inputs.iter_mut().for_each(|arg| {
        if let FnArg::Typed(arg) = arg {
            arg.pat = Box::new(generate_unique_pattern((*arg.pat).clone(), &mut generated_pattern_counter));
        }
    });
}

/// Fold the given `Signature` to make it usable on the client side.
pub fn fold_fn_decl_for_client_side(input: &mut Signature, block_hash: &TokenStream, crate_: &TokenStream) {
...
pub fn extract_block_type_from_trait_path(trait_: &Path) -> Result<&TypePath> {
    ...
    let generics = trait_.segments.last().ok_or_else(|| Error::new(span, "Empty path not supported"))?;
    ...
    impl_
        ...
        .and_then(|p| {
            if p.segments.len() > 1 || matches!(require, RequireQualifiedTraitPath::No) {
                ...
                Err(Error::new(
                    p.span(),
                    "The implemented trait has to be referenced with a path, e.g. `impl client::Core for Runtime`.",
                ))
            }
        })
}
