...
//! the [`RuntimeLogger`](mp_runtime::runtime_logger::RuntimeLogger). This runtime logger is
...
#[doc(hidden)]
#[cfg(feature = "std")]
pub use mp_runtime::StateVersion;
#[doc(hidden)]
pub use mp_runtime::{
    generic::BlockId,
    traits::{
...
#[doc(hidden)]
pub use mp_version::{create_apis_vec, ApiId, ApisVec, RuntimeVersion};
#[doc(hidden)]
pub use scale_info;
#[doc(hidden)]
#[cfg(not(feature = "std"))]
...
pub use sp_state_machine::{
    backend::AsTrieBackend, Backend as StateBackend, InMemoryBackend, OverlayedChanges, StorageProof, TrieBackend,
    TrieBackendBuilder,
};
#[doc(hidden)]
...
/// the versions before.
pub use mp_api_proc_macro::decl_runtime_apis;
...
///
/// ```rust
/// use mp_version::create_runtime_str;
/// #
/// # use mp_runtime::traits::{GetNodeBlockType, Block as BlockT};
...
/// pub const VERSION: mp_version::RuntimeVersion = mp_version::RuntimeVersion {
...
pub use mp_api_proc_macro::impl_runtime_apis;
...
///
/// Besides implementing the given traits, the [`Core`](mp_api::Core) and
/// [`ApiExt`](mp_api::ApiExt) are implemented automatically.
///
/// # Example
///
/// ```rust
...
/// function is called with the `at` parameter that is a [`Hash`](mp_runtime::traits::Hash).
...
///     impl Balance<Block> for MockApi {
///         #[advanced]
///         fn get_balance(&self, at: <Block as BlockT>::Hash) -> Result<u64, mp_api::ApiError> {
///             println!("Being called at: {}", at);
///
///             Ok(self.balance.into())
///         }
///         #[advanced]
///         fn set_balance(at: <Block as BlockT>::Hash, val: u64) -> Result<(), mp_api::ApiError> {
///             println!("Being called at: {}", at);
///
///             Ok(().into())
///         }
...
/// # fn main() {}
/// ```
pub use mp_api_proc_macro::mock_impl_runtime_apis;
...
pub type StorageTransactionCache<Block, Backend> =
    sp_state_machine::StorageTransactionCache<<Backend as StateBackend<HashFor<Block>>>::Transaction, HashFor<Block>>;

#[cfg(feature = "std")]
pub type StorageChanges<SBackend, Block> =
    sp_state_machine::StorageChanges<<SBackend as StateBackend<HashFor<Block>>>::Transaction, HashFor<Block>>;
...
/// Init the [`RuntimeLogger`](mp_runtime::runtime_logger::RuntimeLogger).
pub fn init_runtime_logger() {
    #[cfg(not(feature = "disable-logging"))]
    mp_runtime::runtime_logger::RuntimeLogger::init();
}
...
pub trait CallApiAt<Block: BlockT> {
    ...
    /// the encoded result.
    fn call_api_at(&self, params: CallApiAtParams<Block, Self::StateBackend>) -> Result<Vec<u8>, ApiError>;
...
pub fn deserialize_runtime_api_info(bytes: [u8; RUNTIME_API_INFO_SIZE]) -> ([u8; 8], u32) {
    let id: [u8; 8] = bytes[0..8].try_into().expect("the source slice size is equal to the dest array length; qed");

    let version =
        u32::from_le_bytes(bytes[8..12].try_into().expect("the source slice size is equal to the array length; qed"));
