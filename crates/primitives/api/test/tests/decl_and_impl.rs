...
use mp_api::{decl_runtime_apis, impl_runtime_apis, mock_impl_runtime_apis, ApiError, ApiExt, RuntimeApiInfo};
...
fn test_client_side_function_signature() {
    let _test: fn(&RuntimeApiImpl<Block, TestClient>, <Block as BlockT>::Hash, u64) -> Result<(), ApiError> =
...
fn check_runtime_api_info() {
    ...
    assert_eq!(<dyn ApiWithCustomVersion::<Block>>::VERSION, runtime_decl_for_api_with_custom_version::VERSION,);
    assert_eq!(&<dyn ApiWithCustomVersion::<Block>>::ID, &runtime_decl_for_api_with_custom_version::ID,);
...
fn check_runtime_api_versions() {
    ...
    assert!(RUNTIME_API_VERSIONS.iter().any(|v| v == &(<dyn ApiWithMultipleVersions<Block>>::ID, 3)));
    check_runtime_api_versions_contains::<dyn mp_api::Core<Block>>();
}
...
fn mock_runtime_api_works_with_advanced() {
    ...
    assert_eq!("Test error".to_string(), mock.wild_card(Hash::repeat_byte(0x01), 1).unwrap_err().to_string(),);
}
