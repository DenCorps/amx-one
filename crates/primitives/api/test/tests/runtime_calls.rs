...
use codec::Encode;
use mc_block_builder::BlockBuilderProvider;
use mp_api::{Core, ProvideRuntimeApi};
use mp_runtime::traits::{HashFor, Header as HeaderT};
use mp_consensus::SelectChain;
...
use substrate_test_runtime_client::runtime::{Block, Header, TestAPI, Transfer};
...
fn record_proof_works() {
    let (client, longest_chain) =
        TestClientBuilder::new().set_execution_strategy(ExecutionStrategy::Both).build_with_longest_chain();

    let storage_root = *futures::executor::block_on(longest_chain.best_chain()).unwrap().state_root();
    ...
    let transaction =
        Transfer { amount: 1000, nonce: 0, from: AccountKeyring::Alice.into(), to: AccountKeyring::Bob.into() }
            .into_unchecked_extrinsic();

    // Build the block and record proof
    let mut builder =
        client.new_block_at(client.chain_info().best_hash, Default::default(), true).expect("Creates block builder");
    ...
    let executor =
        NativeElseWasmExecutor::<LocalExecutorDispatch>::new_with_wasm_executor(WasmExecutor::builder().build());
...
fn call_runtime_api_with_multiple_arguments() {
    ...
    let best_hash = client.chain_info().best_hash;
    client.runtime_api().test_multiple_arguments(best_hash, data.clone(), data.clone(), data.len() as u32).unwrap();
}
...
fn disable_logging_works() {
    if std::env::var("RUN_TEST").is_ok() {
        ...
        let mut builder = TestClientBuilder::new().set_execution_strategy(ExecutionStrategy::AlwaysWasm);
        builder
            .genesis_init_mut()
            .set_wasm_code(substrate_test_runtime_client::runtime::wasm_binary_logging_disabled_unwrap().to_vec());
