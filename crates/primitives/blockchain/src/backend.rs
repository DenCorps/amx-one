...
use mp_runtime::traits::{Block as BlockT, Header as HeaderT, NumberFor, Saturating};
...
pub trait HeaderBackend<Block: BlockT>: Send + Sync {
    ...
    fn expect_header(&self, hash: Block::Hash) -> Result<Block::Header> {
        self.header(hash)?.ok_or_else(|| Error::UnknownBlock(format!("Expect header: {}", hash)))
    }
    ...
    fn expect_block_hash_from_id(&self, id: &BlockId<Block>) -> Result<Block::Hash> {
        self.block_hash_from_id(id)
            .and_then(|h| h.ok_or_else(|| Error::UnknownBlock(format!("Expect block hash from id: {}", id))))
    }
}
    ...
    fn longest_containing(&self, base_hash: Block::Hash, import_lock: &RwLock<()>) -> Result<Option<Block::Hash>> {
        ...
        for leaf_hash in leaves {
            ...
            loop {
                ...
                let current_header =
                    self.header(current_hash)?.ok_or_else(|| Error::MissingHeader(current_hash.to_string()))?;
        ...
        warn!("Block {:?} exists in chain but not found when following all leaves backwards", base_hash,);
