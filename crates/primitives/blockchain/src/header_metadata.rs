...
use lru::LruCache;
use mp_runtime::traits::{Block as BlockT, Header, NumberFor, One};
...
impl<Block: BlockT> TreeRoute<Block> {
    ...
    pub fn new(route: Vec<HashAndNumber<Block>>, pivot: usize) -> Result<Self, String> {
        if pivot < route.len() {
            ...
            Err(format!("TreeRoute pivot ({}) should be less than route length ({})", pivot, route.len()))
        }
    }
    ...
    pub fn common_block(&self) -> &HashAndNumber<Block> {
        self.route.get(self.pivot).expect(
            "tree-routes are computed between blocks; which are included in the route; thus it is never empty; qed",
        )
    }
