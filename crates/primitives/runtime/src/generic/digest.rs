...
use crate::scale_info::build::{Fields, Variants};
...
mod tests {
    ...
    fn should_serialize_digest() {
        let digest = Digest { logs: vec![DigestItem::Other(vec![1, 2, 3]), DigestItem::Seal(*b"test", vec![1, 2, 3])] };

        assert_eq!(serde_json::to_string(&digest).unwrap(), r#"{"logs":["0x000c010203","0x05746573740c010203"]}"#);
    }
    ...
    fn digest_item_type_info() {
        ...
        let check = |digest_item_type: DigestItemType| {
            let (variant_name, digest_item) = match digest_item_type {
                ...
                DigestItemType::Seal => ("Seal", DigestItem::Seal(Default::default(), Default::default())),
                DigestItemType::PreRuntime => {
            ...
            let variant =
                variants.iter().find(|v| v.name == variant_name).expect(&format!("Variant {} not found", variant_name));
