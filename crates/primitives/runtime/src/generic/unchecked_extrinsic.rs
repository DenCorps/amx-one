...
use scale_info::{meta_type, Path, StaticTypeInfo, Type, TypeInfo, TypeParameter};
...
use crate::generic::CheckedExtrinsic;
use crate::traits::{
    self, Checkable, Extrinsic, ExtrinsicMetadata, IdentifyAccount, MaybeDisplay, Member, SignedExtension,
};
use crate::transaction_validity::{InvalidTransaction, TransactionValidityError};
    ...
    Extra: SignedExtension,
{
    ...
    fn using_encoded<R, F: FnOnce(&[u8]) -> R>(&self, f: F) -> R {
        self.0.using_encoded(|payload| if payload.len() > 256 { f(&blake2_256(payload)[..]) } else { f(payload) })
    }
}
    ...
    Extra: SignedExtension,
{
    fn from(extrinsic: UncheckedExtrinsic<Address, Call, Signature, Extra>) -> Self {
        Self::from_bytes(extrinsic.encode().as_slice()).expect(
            "both OpaqueExtrinsic and UncheckedExtrinsic have encoding that is compatible with raw Vec<u8> encoding; \
             qed",
        )
    }
}
...
mod tests {
    ...
    use crate::traits::{DispatchInfoOf, IdentityLookup, SignedExtension};
    ...
    fn large_signed_codec_should_work() {
        let ux = Ex::new_signed(
            vec![0u8; 0],
            TEST_ACCOUNT,
            TestSig(TEST_ACCOUNT, (vec![0u8; 257], TestExtra).using_encoded(blake2_256)[..].to_owned()),
            TestExtra,
        );
        let encoded = ux.encode();
        assert_eq!(Ex::decode(&mut &encoded[..]), Ok(ux));
    ...
    fn badly_signed_check_should_fail() {
        let ux = Ex::new_signed(vec![0u8; 0], TEST_ACCOUNT, TestSig(TEST_ACCOUNT, vec![0u8; 0]), TestExtra);
        assert!(ux.is_signed().unwrap_or(false));
        assert_eq!(
