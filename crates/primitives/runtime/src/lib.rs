...
use sp_core::crypto::{self, ByteArray};
...
pub use sp_arithmetic::{
    traits::SaturatedConversion, ArithmeticError, FixedI128, FixedI64, FixedPointNumber, FixedPointOperand, FixedU128,
    InnerOf, PerThing, PerU16, Perbill, Percent, Permill, Perquintill, Rational128, Rounding, UpperOf,
};
...
impl Verify for AnySignature {
    ...
    fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &sr25519::Public) -> bool {
        let msg = msg.get();
        sr25519::Signature::try_from(self.0.as_fixed_bytes().as_ref()).map(|s| s.verify(msg, signer)).unwrap_or(false)
...
/// ```rust
/// # fn main() {
/// mp_runtime::assert_eq_error_rate!(10, 10, 0);
/// mp_runtime::assert_eq_error_rate!(10, 11, 1);
/// mp_runtime::assert_eq_error_rate!(12, 10, 2);
/// # }
/// ```
///
/// ```rust,should_panic
/// # fn main() {
/// mp_runtime::assert_eq_error_rate!(12, 10, 1);
/// # }
/// ```
#[macro_export]
#[cfg(feature = "std")]
...
macro_rules! assert_eq_error_rate_float {
    ($x:expr, $y:expr, $error:expr $(,)?) => {
        assert!(($x >= $y - $error) && ($x <= $y + $error), "{:?} != {:?} (with error rate {:?})", $x, $y, $error,);
    };
}
...
mod tests {
    ...
    fn dispatch_error_encoding() {
        let error =
            DispatchError::Module(ModuleError { index: 1, error: [2, 0, 0, 0], message: Some("error message") });
        let encoded = error.encode();
        let decoded = DispatchError::decode(&mut &encoded[..]).unwrap();
        assert_eq!(encoded, vec![3, 1, 2, 0, 0, 0]);
        assert_eq!(decoded, DispatchError::Module(ModuleError { index: 1, error: [2, 0, 0, 0], message: None }));
    }
