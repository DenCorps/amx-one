...
//! use mp_runtime::offchain::storage_lock::{StorageLock, Time};
//!
...
use crate::offchain::storage::{MutateStorageError, StorageRetrievalError, StorageValueRef};
...
impl<B: BlockNumberProvider> Lockable for BlockAndTime<B> {
    ...
    fn deadline(&self) -> Self::Deadline {
        ...
        BlockAndTimeDeadline { timestamp: offchain::timestamp().add(self.expiration_duration), block_number }
    }
        ...
        let res = self.value_ref.mutate(
            |s: Result<Option<L::Deadline>, StorageRetrievalError>| -> Result<<L as Lockable>::Deadline, ()> {
                match s {
                    ...
                    Ok(Some(deadline)) if !<L as Lockable>::has_expired(&deadline) => Ok(self.lockable.deadline()),
                    // other cases
                    _ => Err(()),
                }
            },
...
impl<'a, 'b, L: Lockable> StorageLockGuard<'a, 'b, L> {
    ...
    pub fn extend_lock(&mut self) -> Result<<L as Lockable>::Deadline, ()> {
        if let Some(ref mut lock) = self.lock { lock.extend_active_lock() } else { Err(()) }
    }
}
...
impl<'a> StorageLock<'a, Time> {
    ...
    pub fn with_deadline(key: &'a [u8], expiration_duration: Duration) -> Self {
        Self { value_ref: StorageValueRef::<'a>::persistent(key), lockable: Time { expiration_duration } }
    }
}
