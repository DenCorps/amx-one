...
pub use sp_arithmetic::traits::{
    checked_pow, ensure_pow, AtLeast32Bit, AtLeast32BitUnsigned, Bounded, CheckedAdd, CheckedDiv, CheckedMul,
    CheckedShl, CheckedShr, CheckedSub, Ensure, EnsureAdd, EnsureAddAssign, EnsureDiv, EnsureDivAssign,
    EnsureFixedPointNumber, EnsureFrom, EnsureInto, EnsureMul, EnsureMulAssign, EnsureOp, EnsureOpAssign, EnsureSub,
    EnsureSubAssign, IntegerSquareRoot, One, SaturatedConversion, Saturating, UniqueSaturatedFrom, UniqueSaturatedInto,
    Zero,
};
...
#[doc(hidden)]
pub use sp_core::{
    parameter_types, ConstBool, ConstI128, ConstI16, ConstI32, ConstI64, ConstI8, ConstU128, ConstU16, ConstU32,
...
use crate::codec::{Codec, Decode, Encode, MaxEncodedLen};
use crate::generic::Digest;
use crate::scale_info::{MetaType, StaticTypeInfo, TypeInfo};
...
impl<
    S: Verify<Signer = <<T as AppCrypto>::Public as sp_application_crypto::AppPublic>::Generic> + From<T>,
...
where
    ...
    <<T as AppCrypto>::Public as sp_application_crypto::AppPublic>::Generic:
        IdentifyAccount<AccountId = <<T as AppCrypto>::Public as sp_application_crypto::AppPublic>::Generic>,
{
    ...
    fn verify<L: Lazy<[u8]>>(&self, msg: L, signer: &<T as AppCrypto>::Public) -> bool {
        ...
        let inner_pubkey = <<T as AppCrypto>::Public as sp_application_crypto::AppPublic>::Generic::from_ref(signer);
...
///
/// ```
/// # use mp_runtime::{morph_types, traits::{Morph, TryMorph, TypedGet, ConstU32}};
...
pub trait Dispatchable {
    ...
    fn dispatch(self, origin: Self::RuntimeOrigin) -> crate::DispatchResultWithInfo<Self::PostInfo>;
}
...
impl Dispatchable for () {
    ...
    type Info = ();
    type PostInfo = ();
    fn dispatch(self, _origin: Self::RuntimeOrigin) -> crate::DispatchResultWithInfo<Self::PostInfo> {
...
impl<T: Encode + Decode, Id: Encode + Decode + TypeId> AccountIdConversion<T> for Id {
    ...
    fn try_into_sub_account<S: Encode>(&self, sub: S) -> Option<T> {
        ...
        let account =
            T::decode(&mut TrailingZeroInput(&encoded_seed)).expect("All byte sequences are valid `AccountIds`; qed");
        // If the `account` generated has less bytes than the `encoded_seed`, then we know that
        // bytes were truncated, and we return `None`.
        if encoded_seed.len() <= account.encoded_size() { Some(account) } else { None }
    }
    ...
    fn try_from_sub_account<S: Decode>(x: &T) -> Option<(Self, S)> {
        x.using_encoded(|d| {
            ...
            if cursor.iter().all(|x| *x == 0) { Some(result) } else { None }
        })
    }
}
...
///
/// ```rust
/// use mp_runtime::app_crypto::{ed25519, sr25519};
/// use mp_runtime::{impl_opaque_keys, BoundToRuntimeAppPublic, KeyTypeId};
///
...
impl Printable for bool {
    fn print(&self) {
        if *self { "true".print() } else { "false".print() }
    }
}
...
pub trait BlockNumberProvider {
    ...
    ///
    /// In case of using crate `mp_runtime` with the crate `frame-system`,
...
mod tests {
    ...
    fn try_into_sub_account_should_work() {
        ...
        let maybe: Option<AccountId> = U16Value::try_into_sub_account(
            &U16Value(0xc0da),
            "a really large amount of additional encoded information which will certainly overflow the account id \
             type ;)",
        );
