...
use syn::{parse_macro_input, Expr, ExprLit, FieldValue, ItemConst, Lit};
...
pub fn decl_runtime_version_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    decl_runtime_version_impl_inner(item).unwrap_or_else(|e| e.to_compile_error()).into()
}
...
/// This is a duplicate of `mp_version::RuntimeVersion`. We cannot unfortunately use the original
/// declaration, because if we directly depend on `mp_version` from this proc-macro cargo will
...
impl ParseRuntimeVersion {
    fn parse_expr(init_expr: &Expr) -> Result<ParseRuntimeVersion> {
        let init_expr = match init_expr {
            Expr::Struct(ref e) => e,
            _ => return Err(Error::new(init_expr.span(), "expected a struct initializer expression")),
        };
    ...
    fn parse_field_value(&mut self, field_value: &FieldValue) -> Result<()> {
        let field_name = match field_value.member {
            ...
            syn::Member::Unnamed(_) => return Err(Error::new(field_value.span(), "only named members must be used")),
        };
    ...
    fn parse_num_literal(expr: &Expr) -> Result<u32> {
        let lit = match *expr {
            ...
            _ => return Err(Error::new(expr.span(), "only numeric literals (e.g. `10`) are supported here")),
        };
    ...
    fn parse_str_literal(expr: &Expr) -> Result<String> {
        ...
        let lit: ExprLit = mac.parse_body().map_err(|e| {
            Error::new(e.span(), format!("a single literal argument is expected, but parsing is failed: {}", e))
        })?;
    ...
    fn build(self, span: Span) -> Result<RuntimeVersion> {
        macro_rules! required {
            ($e:expr) => {
                $e.ok_or_else(|| Error::new(span, format!("required field '{}' is missing", stringify!($e))))?
            };
        }
...
mod tests {
    ...
    fn version_can_be_deserialized() {
        ...
        assert_eq!(
            mp_version::RuntimeVersion::decode_with_version_hint(&mut &version_bytes[..], Some(4)).unwrap(),
            mp_version::RuntimeVersion {
