...
///
/// ```rust
/// use mp_version::{create_runtime_str, RuntimeVersion};
///
...
/// };
///
/// # const RUNTIME_API_VERSIONS: mp_version::ApisVec = mp_version::create_apis_vec!([]);
/// ```
///
...
///
/// - `authoring_version`, `spec_version`, `impl_version` and `transaction_version` must be set by a
///   literal. Literal must be an integer. No other expressions are allowed there. In particular,
///   you can't supply a constant variable.
///
/// - `apis` doesn't have any specific constraints. This is because this information doesn't get
///   into the custom section and is not parsed.
///
...
pub use mp_version_proc_macro::runtime_version;
...
/// runtime.
#[derive(Clone, PartialEq, Eq, Encode, Default, mp_runtime::RuntimeDebug, TypeInfo)]
...
pub struct RuntimeVersion {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(serialize_with = "apis_serialize::serialize", deserialize_with = "apis_serialize::deserialize",)
    )]
    pub apis: ApisVec,
...
impl RuntimeVersion {
    ...
    pub fn decode_with_version_hint<I: Input>(
        ...
        let core_version = if core_version.is_some() { core_version } else { core_version_from_apis(&apis) };
        let transaction_version =
            if core_version.map(|v| v >= 3).unwrap_or(false) { Decode::decode(input)? } else { 1 };
        let state_version = if core_version.map(|v| v >= 4).unwrap_or(false) { Decode::decode(input)? } else { 0 };
        Ok(RuntimeVersion {
            spec_name,
            impl_name,
            authoring_version,
...
impl NativeVersion {
    ...
    pub fn can_author_with(&self, other: &RuntimeVersion) -> Result<(), String> {
        if self.runtime_version.spec_name != other.spec_name {
            Err(format!("`spec_name` does not match `{}` vs `{}`", self.runtime_version.spec_name, other.spec_name,))
        ...
        {
            Err(format!(
                "`authoring_version` does not match `{version}` vs `{other_version}` and `can_author_with` not \
                 contains `{other_version}`",
...
#[cfg(feature = "std")]
impl<T: GetRuntimeVersionAt<Block>, Block: BlockT> GetRuntimeVersionAt<Block> for std::sync::Arc<T> {
    fn runtime_version(&self, at: <Block as BlockT>::Hash) -> Result<RuntimeVersion, String> {
